...
class Table {
  constructor(_scene) {
    ...
    this.tableCardConstantValueObj = {
      numberOfCards: 5,
      tableCardScale: 0.85,
      tableCardStartPosX: -136,
      // tableCardStartPosX: 28,
      tableCardStartPosY: -128,
      tableCardGapX: -6.84,
  ...
  CreateTableBg() {
    this.tableBg = this.scene.add.image(0, 0, 'table_bg_0').setOrigin(0.5);
  ...
  CreatePotValueArea() {
    ...
    // this.animChip = this.scene.add.image(this.potValueChip.x, this.potValueChip.y, 'pot_value_chip').setOrigin(0.5).setVisible(false);
    ...
    const potValueTextStyle = { fontFamily: 'Poppins-Medium', fontSize: '24px', fill: '#FFF3F2', fontStyle: 'bold', align: 'center', wordWrap: { width: this.potValueBase.width - 5 } };
    this.potValueText = this.scene.add.text(this.potValueBase.x, this.potValueBase.y + 1, '', potValueTextStyle).setOrigin(0.5);
  ...
  CreatePotSplitArea(_arr) {
    ...
    const initialPosY = this.potValueBase.y + 45;
    const gap = this.potValueBase.displayWidth + (3 * (initialPosX - (this.potValueBase.displayWidth / 2)));
    if (_arr.length === 1) {
      const potBase = this.scene.add.image(0, 0, 'pot_value_base').setOrigin(0.5).setScale(this.potSplitScale);
      potBase.setPosition(initialPosX, initialPosY);
      const potValueChip = this.scene.add.image(potBase.x - 67.5, potBase.y + 8, 'pot_value_chip').setOrigin(0.5).setScale(this.potSplitScale);
      const potValueTextStyle = { fontFamily: 'Poppins-Medium', fontSize: '24px', fill: '#FFF3F2', fontStyle: 'bold', align: 'center', wordWrap: { width: potBase.width - 5 } };
      const potAmtTextFormat = _arr[0].val >= 1000 ? `${(parseFloat(_arr[0].val) / 1000).toFixed(2)}k` : parseFloat(_arr[0].val).toFixed(2);
      const potAmtText = this.scene.add.text(potBase.x, potBase.y + 1, potAmtTextFormat, potValueTextStyle).setOrigin(0.5).setScale(this.potSplitScale);
      this.potSplitArray.push(potAmtText);
      this.potSplitContainer.add([potBase, potValueChip, potAmtText]);
    } else {
      const totalWidth = (_arr.length * this.potValueBase.displayWidth) + ((_arr.length - 1) * gap);
      const startX = initialPosX - (totalWidth / 2) + (this.potValueBase.displayWidth / 2);
      for (let i = 0; i < _arr.length; i++) {
        const potBase = this.scene.add.image(0, 0, 'pot_value_base').setOrigin(0.5).setScale(this.potSplitScale);
        potBase.setPosition(startX + (i * (this.potValueBase.displayWidth + gap)), initialPosY);
        const potValueChip = this.scene.add.image(potBase.x - 67.5, potBase.y + 8, 'pot_value_chip').setOrigin(0.5).setScale(this.potSplitScale);
        const potValueTextStyle = { fontFamily: 'Poppins-Medium', fontSize: '24px', fill: '#FFF3F2', fontStyle: 'bold', align: 'center', wordWrap: { width: potBase.width - 5 } };
  ...
  ShowTableCard(_arr, _roundName, _isViaGameState) {
    switch (_roundName) {
      case 'flop':
        ...
        // this.TableCardPositionAdjust(_arr.length, _isViaGameState);
        // setTimeout(() => {
        //   for (let i = 0; i < _arr.length; i++) {
        //     const cardFrame = Model.GetActualCardFrameIndex(_arr[i].card_id);
        //     this.tableCardArray[i].setVisible(true);
      ...
      case 'turn':
        {
          ...
          // this.TableCardPositionAdjust(4, _isViaGameState);
          // setTimeout(() => {
          //   this.tableCardArray[3].setVisible(true);
          //   this.tableCardArray[3].name = _arr[0].card_id;
          //   this.RotateTableCard(this.tableCardArray[3], cardFrame, _isViaGameState);
      ...
      case 'river':
        {
          ...
          // this.TableCardPositionAdjust(5, _isViaGameState);
          // setTimeout(() => {
          //   this.tableCardArray[4].setVisible(true);
          //   this.tableCardArray[4].name = _arr[0].card_id;
          //   this.RotateTableCard(this.tableCardArray[4], cardFrame, _isViaGameState);
