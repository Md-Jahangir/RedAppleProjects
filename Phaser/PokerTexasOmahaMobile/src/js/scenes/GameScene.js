...
// import ChatPopup from '../popup/ChatPopup.js';
...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.game.events.on('evtSocketConnection', this.TempDebug, this);
    // this.playerPosArray = [
    ...
    //   player.AdjustSmallBlindSection(this.userSmallBigBlindPosArray[i].x, this.userSmallBigBlindPosArray[i].y);
    //   player.AdjustCurrentBetShowArea(this.CurrentBetShowAreaPosArray[i].x, this.CurrentBetShowAreaPosArray[i].y, this.CurrentBetShowAreaPosArray[i].allignment);
    //   //   // player.FoldPlayer();
    ...
    // this.allPlayerArray[0].AdjustSmallBlindSection(this.userSmallBigBlindPosArray[0].x - 20, this.userSmallBigBlindPosArray[0].y);
    // this.allPlayerArray[0].AdjustBigBlindSection(this.userSmallBigBlindPosArray[0].x - 20, this.userSmallBigBlindPosArray[0].y);
    // this.allPlayerArray[0].AdjustPlayerInfoSection(this.userInfoDataArray[0].x, this.userInfoDataArray[0].y + 40, this.userInfoDataArray[0].nameBaseXoffset, this.userInfoDataArray[0].nameTextOrigin, this.userInfoDataArray[0].offsetXUserBalance, this.userInfoDataArray[0].distFactor);
    ...
    // this.allPlayerArray[1].HideDecisionBase(false);
    // this.allPlayerArray[1].FoldPlayer();
    // this.allPlayerArray[1].ShowDecisionBase('Full House', false);
  ...
  TempDebug(_msg) {
    this.tempSocketConnectMsg.setVisible(true);
    this.tempSocketConnectMsg.setText(_msg);
    setTimeout(() => {
      this.tempSocketConnectMsg.setVisible(false);
    }, 2000);
  }
  ...
  ResizeGameplayBg(newWidth, newHeight, newScale) {
    ...
    this.versionControlText.setPosition(50 * newScale, newHeight - (100 * newScale));
  };
  //#endregion
  ...
  ResizeChip(_newWidth, _newHeight, _newScale) {
    for (let i = 0; i < this.chipArray.length; i++) {
      ...
      // this.chipArray[i].setPosition((_newWidth / 2) + ((this.playerPosArray[i].x + this.CurrentBetShowAreaPosArray[i].x) * _newScale), (_newHeight / 2) + ((this.playerPosArray[i].y + this.CurrentBetShowAreaPosArray[i].y) * _newScale));
    }
  };
  ...
  CreatePlayer() {
    ...
    try {
      ...
      if (!Utils.IsEmpty(playersResponseData)) {
        localPlayerIndex = playersResponseData.findIndex(user => user.user_id === localPlayerID);
        // localPlayerIndex = playersResponseData.findIndex(user => user.remote_user_id === localPlayerRemoteID);
        localPlayerSeatIndex = (localPlayerIndex === -1) ? 0 : playersResponseData[localPlayerIndex].seat_no;
  ...
  //#region - JoinPlayer
  JoinPlayer() {
    // const newPlayerData = Model.roomDetailsData.players[Model.roomDetailsData.players.length - 1];
    ...
    //   this.placeHolderContainerArray.forEach((_, i) => this.ShowHidePlaceHolder(i, true));
    // }
    // const localPlayerSeatIndex = playersResponseData[localPlayerIndex].seat_no;
    if (newPlayerData.user_id === localPlayerID && newPlayerData.seat_no !== 0) {
      ...
      if (newPlayerData.sit_in) {
        ...
        player.SetPlayerPositionIndex(positionIndex);
        // player.isDelear = newPlayerData.is_dealer;
        // player.ShowHideDelearIcon(player.isDelear);
  ...
  PlaceBetForCurrentPlayer() {
    ...
    this.TempDebug(this.currentBet);
  ...
  SpectatorPreFlopBackCardShow() {
    if (Model.GetSeatIndex() === null) {
      this.allPlayerArray.forEach(player => {
        console.log('show back card', player);
  ...
  resize(_newWidth, _newHeight) {
    ...
    this.tempSocketConnectMsg.setScale(newScale);
    this.tempSocketConnectMsg.setPosition(_newWidth / 2, 50 * newScale);
