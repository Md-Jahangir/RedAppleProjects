...
if(mode === 'offline'){
  ...
  balanceVars = 'http://localhost:1337/casinos/api?action=balance' + '&mode=' + mode + '&token=' + token;
}else{
...
//var balanceVars = 'https://api.gamessecure.com/games/admin?action=player_balance&token='+token+'&remote_id='+remote_id+'&casino=' + casino
        fetch(balanceVars)
        .then(resp => resp.json())
        .then(function (data) {
          if (data.balance) {
            var oMain = new CMain({
                                    ...
                                    money: data.balance, //STARING CREDIT FOR THE USER
