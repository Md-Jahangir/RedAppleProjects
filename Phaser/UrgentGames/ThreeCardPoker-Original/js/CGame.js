function CGame(oData) {
    ...
    this.shuffleCard = function() {
        ...
        _aCardDeck = serverRequests.getShuffledCardDeck();
    ...
    this.onDeal = function() {
        ...
        var anteBet = _oSeat.getBetAnte();
        var plusBet = _oSeat.getBetPlus();
        var totalBet = anteBet + plusBet;
        if (mode === 'offline') {} else {
            var response = serverRequests.deductBetAmount(totalBet);
            response = parseFloat(response);
            response = parseFloat(response.toFixed(2));
            _oSeat.setCredit(response);
            _oInterface.refreshCredit(_oSeat.getCredit());
        }
        ...
        if (iRandWin > WIN_OCCURRENCE) {
            //LOSE
            do {
                //_aPlayerCardsInfo = serverRequests._generateRandPlayerCards();
                _aPlayerCardsInfo = this._generateRandPlayerCards();
                // _aDealerCardsInfo = serverRequests._generateRandDealerCards();
    ...
    this.onPlay = function() {
        ...
        var playBet = _oSeat.getBetPlay();
        if (mode === 'offline') {} else {
            var response = serverRequests.deductBetAmount(playBet);
            response = parseFloat(response);
            response = parseFloat(response.toFixed(2));
            _oSeat.setCredit(response);
            _oInterface.refreshCredit(_oSeat.getCredit());
