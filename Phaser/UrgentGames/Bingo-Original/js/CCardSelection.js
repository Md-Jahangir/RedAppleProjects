function CCardSelection(iMoney,oParentContainer){
    ...
    this._init = function(iMoney){
        _iMoney = iMoney;
        _iNumCards = s_iCurNumCards;
        _iCurNumBalls = NUM_EXTRACTIONS[s_iCurNumToExtract];

        _iTotBet = COIN_BETS[s_iCurCoinBet] * _iNumCards;
        _iIndexNumBalls = 0;
        ...
        _oCoinText = new CDisplayText(_pStartPosCoin.x,_pStartPosCoin.y,s_oSpriteLibrary.getSprite('display_small'),COIN_BETS[s_iCurCoinBet]+TEXT_CURRENCY,TEXT_COIN,37,_oContainer);
        ...
        if(s_iCurCoinBet === COIN_BETS.length-1){
        ...
        _oButNumBall1 = new CToggleText(s_iCurNumToExtract===0?true:false,CANVAS_WIDTH/2 - oSpriteBallBut.width - 200,700,oSpriteBallBut,oSpriteBallBut.width/2,oSpriteBallBut.height,
        ...
        _oButNumBall2 = new CToggleText(s_iCurNumToExtract===1?true:false,CANVAS_WIDTH/2 - 200,700,oSpriteBallBut,oSpriteBallBut.width/2,oSpriteBallBut.height,
        ...
        _oButNumBall3 = new CToggleText(s_iCurNumToExtract===2?true:false,CANVAS_WIDTH/2 + oSpriteBallBut.width - 200,700,oSpriteBallBut,oSpriteBallBut.width/2,oSpriteBallBut.height,
    ...
    this.setMoney = function(iMoney){
        _iMoney += iMoney;
        _oMoneyText.changeText(_iMoney+TEXT_CURRENCY);
    ...
    this._checkIfCanPlay = function(){
        var bEnoughMoney = this._checkIfEnoughMoney();
        if(!bEnoughMoney){
            ///SET MINIMUM BET
            s_iCurCoinBet = 0;
            _iNumCards = 1;
            
            _oCoinText.changeText(TEXT_CURRENCY+COIN_BETS[s_iCurCoinBet]);
            _iTotBet = (COIN_BETS[s_iCurCoinBet]/COIN_BETS[0]) * (COIN_BETS[0]*_iNumCards);
            _oNumCardsText.text = _iNumCards;
            _oTotBetText.changeText(_iTotBet + TEXT_CURRENCY);
            
            if(this._checkIfEnoughMoney() === false){
                ///IF MINIMUM BET IS NOT ENOUGH
    ...
    this._setBetButtonsStates = function(){
        ...
        ///CHECK NEXT BET AVAILABLE
        if(s_iCurCoinBet<COIN_BETS.length-1){
            var iNextCoinBet = s_iCurCoinBet+1;
            var iNextTotBet = (COIN_BETS[iNextCoinBet]/COIN_BETS[0]) * (COIN_BETS[0]*_iNumCards);
            if(_iMoney < iNextTotBet){
                _oButBetPlus.disable();
            }
        }
    };
    ...
    this._setCardButtonsStates = function(){
        ...
        ///CHECK NEXT CARD AVAILABLE
        if(_iNumCards<MAX_CARDS){
            var iNextCard = _iNumCards+1;
            var iNextTotBet = (COIN_BETS[s_iCurCoinBet]/COIN_BETS[0]) * (COIN_BETS[0]*iNextCard);
            if(_iMoney < iNextTotBet){
                _oButCardsPlus.disable();
            }
        }
    ...
    this._onButPlay = function(){
        ...
        //DECREASE MONEY
        s_iCurNumCards = _iNumCards;
        s_iCurNumToExtract = _iIndexNumBalls;

        s_oCardSelection.unload();
        s_oGame.initGame(_iNumCards,_iCurNumBalls,_iMoney,_iTotBet,_iIndexNumBalls);
    };
...
var s_iCurNumCards = 4;
var s_iCurNumToExtract = 0;
var s_iCurCoinBet = 0;