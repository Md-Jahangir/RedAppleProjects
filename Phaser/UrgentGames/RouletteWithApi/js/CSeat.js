function CSeat() {
    ...
    //const mode = urlParams.get('mode')==null?'offline':urlParams.get('mode');
    ...
    this.setFicheBetted = function(iFicheValue, aNumbers, iWinForBet, aFichesMc, iNumFiches) {
        ...
        // console.log("iFicheValue: " + iFicheValue);
        // console.log("iNumFiches: " + iNumFiches);
        ...
        if (mode == 'offline') {
            ...
            // _iCredit = serverRequests.deductBetAmount(iAmount);
            // _iCredit = roundDecimal(_iCredit, 1);
        }
        s_oGame.updateBettedFiches({ num: iNumFiches, value: iFicheValue });
    ...
    this.clearLastBet = function() {
        ...
        for (var i = 0; i < aLastNums.length; i++) {
            if (_aLastBetWinHistory.length > 0) {
                ...
                // _aNumberBetted[aLastNums[i]] = { win: _aNumberBetted[aLastNums[i]].win - aLastWin[i + 1], mc: oTmp.mc };
            } else {
                _aNumberBetted[aLastNums[i]] = { win: _aNumberBetted[aLastNums[i]].win - aLastWin[i], mc: null };
                // _aNumberBetted[aLastNums[i]] = { win: _aNumberBetted[aLastNums[i]].win - aLastWin[i + 1], mc: null };
            }
    ...
    this.clearAllBets = function() {
        s_oGame.updateBettedFichesEmpty();
        this.resetNumberWins();
