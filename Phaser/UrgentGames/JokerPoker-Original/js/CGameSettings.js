function CGameSettings() {
    ...
    this.getShuffledCardDeck = function() {
        ...
        $.ajax({
            //    url: BASE_URL+'?game_id=109&action=getShuffledCardDeck&isGameLogic=true',  
            url: BASE_URL + '?game_id=123&action=getShuffledCardDeck&isGameLogic=true',

            async: false,

            success: function(data) {
                aTmpDeck = data.result;
    ...
    this.getShuffledDeckForDouble = function() {
        ...
        var _aDeckForDoubleBet = new Array();

        $.ajax({
            //    url: BASE_URL+'?game_id=109&action=getShuffledDeckForDouble&isGameLogic=true',  
            url: BASE_URL + '?game_id=123&action=getShuffledDeckForDouble&isGameLogic=true',

            async: false,

            success: function(data) {
                _aDeckForDoubleBet = data.result;
