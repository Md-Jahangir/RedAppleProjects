function CHandEvaluator() {
    ...
    this.evaluate = function(aHand) {
        ...
        var iRet = this.rankHand(_aSortedHand);
        // var iRet = this.rankHand();

        return iRet;
    };

    this.rankHand = function(_aSortedHand) {
        var rankVariable;

        let sendArray = JSON.stringify(_aSortedHand);
        $.ajax({
            // url: BASE_URL+'?game_id=109&action=rankHand&isGameLogic=true&handArr='+sendArray,  
            url: BASE_URL + '?game_id=123&action=rankHand&isGameLogic=true&handArr=' + sendArray,
            //dataType: "json",

            //data: {handArr:_aSortedHand},
            async: false,

            success: function(data) {
                rankVariable = data.result;
