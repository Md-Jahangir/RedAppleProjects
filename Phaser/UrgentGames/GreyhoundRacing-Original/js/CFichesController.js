function CFichesController(iScale,oParentContainer){
    ...
    this.refreshFiches = function(aFiches,iXPos,iYPos){
        ...
        _oBetAmountText.text = _iValue.toFixed(languageService.serverCurrency('currencyDecimils')) + languageService.serverCurrency("currencySymbol");
        _oBetAmountBackText.x = iXPos + 2;
        _oBetAmountBackText.y = iYPos + (27*_iCurScale);
        _oBetAmountBackText.text = _iValue.toFixed(languageService.serverCurrency('currencyDecimils')) + languageService.serverCurrency("currencySymbol");
    };
    ...
    this.createFichesPile = function(iAmount,iX,iY){
        ...
        do{
            ...
            for(var i=0;i<iNumFiches;i++){
                aFichesPile.push({value:iMinValue,index:serverRequests.getIndexForFiches(iMinValue)});
