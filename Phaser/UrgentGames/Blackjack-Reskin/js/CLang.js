...
LanguageService.prototype.init = async function () {
	const translations = await serverRequests.getTranslations();
	this.languageData = translations.translationBlackjack ? translations.translationBlackjack : null;
	this.languageErrorsData = translations.translationErrors ? translations.translationErrors : null;

    this.serverCurrencyData = await serverRequests.getCurrencies();
    this.isReady = true;
    return true;
};
