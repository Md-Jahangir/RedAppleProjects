function CCard(iX, iY, oContainer, card, gameScene) {
    var _bDealerCard;
    var _bRemoving;
    var _iState = -1;
    var _szFotogram;
    ...
    var _card = card;
    var config;
    this._init = function(iX, iY, oContainer, _card) {
        config = orientation_mode[getOrientation()].CCard;
        ...
        _oCardSprite.x = iX;
        _oCardSprite.y = iY;
        _oCardSprite.scaleX = GAME_SCALE * config._oCardSprite.scale
        _oCardSprite.scaleY = GAME_SCALE * config._oCardSprite.scale
        _oCardSprite.stop();
        _oContainer.addChild(_oCardSprite);
        _aCbCompleted = new Array();
        _aCbOwner = new Array();
        _oCardSprite.owner = _card;
        _oCardSprite.setScale = (newScale) => {
            _oCardSprite.scaleX = newScale;
            _oCardSprite.scaleY = newScale;
            _oCardSprite.width = CARD_WIDTH * _oCardSprite.scaleX;
            _oCardSprite.height = CARD_HEIGHT * _oCardSprite.scaleY;
        }
    ...
    //TODO: add force hand flag and force hand index
    this.setInfo = function(pStartingPoint, pEndingPoint, szFotogram, iValue, bDealerCard, iCountCard, isForceHand = false, forceHandIndex) {
        _bRemoving = false;
        _iTimeElaps = 0;
        ...
        _isForceHand = isForceHand;
        _forceHandIndex = forceHandIndex;
    ...
    this.setValue = function() {
        ...
        createjs.Tween.get(_oCardSprite).to({ scaleX: 0.1 * GAME_SCALE }, 100).call(function() { oParent.cardShown() });
    };
    ...
    this.showCard = function() {
                ...
                _oCardSprite.scaleX = portraitScale * 1.5;
                _oCardSprite.scaleY = portraitScale * 1.5;
              } else {
                _oCardSprite.scaleX = GAME_SCALE * config._oCardSprite.scale
                _oCardSprite.scaleY = GAME_SCALE * config._oCardSprite.scale
              }
        ...
        var oParent = this;
        createjs.Tween.get(_oCardSprite).to({ scaleX: 0.1 * GAME_SCALE }, 150).call(function() { oParent.setValue() });
    };

    this.hideCard = function() {
        var oParent = this;
        createjs.Tween.get(_oCardSprite).to({ scaleX: 0.1 * GAME_SCALE}, 100).call(function() { oParent.setBack() });
    };

    this.setBack = function() {
        _oCardSprite.gotoAndStop("back");
        var oParent = this;
        createjs.Tween.get(_oCardSprite).to({ scaleX: 0.1 * GAME_SCALE}, 100).call(function() { oParent.cardHidden() });
    };
        ...
        if (_iTimeElaps > TIME_CARD_DEALING) {
            ...
            if (_aCbCompleted[ON_CARD_ANIMATION_ENDING]) {
                _aCbCompleted[ON_CARD_ANIMATION_ENDING].call(_aCbOwner[ON_CARD_ANIMATION_ENDING],
                    this, _bDealerCard, _iCountCard, _isForceHand, _forceHandIndex);
            }
    ...
    this._init(iX, iY, oContainer, _card);
