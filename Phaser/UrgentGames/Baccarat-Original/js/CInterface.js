function CInterface(iMoney) {
    ...
    this.home_link = this.urlParams.get('home');
    ...
    this._init = function (iMoney) {
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            ...
            _pStartPosInfo = { x: _pStartPosAudio.x - 55, y: _pStartPosAudio.y };
        } else {
            if (this.home_link) {
                _pStartPosInfo = { x: _pStartPosExit.x - 55, y: _pStartPosExit.y };
            } else {
                _pStartPosInfo = { x: CANVAS_WIDTH - (oSprite.width / 2) - 10, y: (oSprite.height / 2) + 10 };
            }
        }
        ...
        var oSprite = s_oSpriteLibrary.getSprite('info_icon');
        ...
        _oButInfo.addEventListener(ON_MOUSE_UP, this.onShowInfoPanel, this);
        ...
        _oClearBetBut = new CGuiButton(939, CANVAS_HEIGHT - 31, oSprite, languageService.getString("TEXT_CLEAR"), FONT_GAME_1, "#ffffff", 17, s_oStage);
        _oClearBetBut.addEventListener(ON_MOUSE_UP, this._onButClearRelease, this);

        oSprite = s_oSpriteLibrary.getSprite('but_rebet');
        _oRebetBut = new CGuiButton(1112, CANVAS_HEIGHT - oSprite.height / 2, oSprite, languageService.getString("TEXT_REBET"), FONT_GAME_1, "#ffffff", 17, s_oStage);
        _oRebetBut.disable();
        ...
        _oMoneyText = new CTLText(s_oStage,
            ...
            0, 0,
            Number(iMoney).toFixed(CURRENCY_DECIMAL) + languageService.getCurrencySymbol(),
            true, true, false,
            false);


        oSprite = s_oSpriteLibrary.getSprite('but_deal');
        _oDealBut = new CGuiButton(1282, CANVAS_HEIGHT - oSprite.height / 2, oSprite, languageService.getString("TEXT_DEAL"), FONT_GAME_1, "#ffffff", 26, s_oStage);
    ...
    this.onShowInfoPanel = function () {
        if (infoPanel === null) {
            infoPanel = new InfoPanel(s_oStage);
            sizeHandler();
        }        
        infoPanel.show();
    }
    ...
    this.refreshButtonPos = function (iNewX, iNewY) {
        ...
        if (infoPanel) infoPanel.refreshButtonPos(s_iOffsetX, s_iOffsetY);
    };
    ...
    this.gameExit = function () {
        _oButExit.unload();
        if (this.home_link) {
            location.replace(this.home_link);
        } else {
    ...
    this.refreshCredit = function (iMoney) {
        _oMoneyText.refreshText(languageService.getCurrencySymbol() + Number(iMoney).toFixed(CURRENCY_DECIMAL));
    };
    ...
    this.showWin = function (iIndex, iWin, szTextWin) {
        var obj = ((languageService.getString("TEXT_SHOW_WIN")));
        _aWinText[iIndex].show(Object.entries(obj)[iIndex][1], Number(iWin).toFixed(CURRENCY_DECIMAL), szTextWin);
    };
