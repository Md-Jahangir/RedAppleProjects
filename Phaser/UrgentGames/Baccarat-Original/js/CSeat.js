function CSeat() {
    ...
    const token = urlParams.get('token') || 'fun';
    ...
    this.setPrevFichesValues = function () {
    ...
    this.bet = function (iFicheValue, iTypeBet, iFicheIndex) {
        ...
        _aFichesController[iTypeBet].createFichesPile(iValue.toFixed(CURRENCY_DECIMAL), POS_BET[iTypeBet].x, POS_BET[iTypeBet].y);
    };



    this.decreaseCreditOld = function (iCreditToSubtract) {
    ...
    this.rebet = function () {
        var iTotBet = 0;
        for (var i = 0; i < _aFichesController.length; i++) {
            var iValue = parseFloat(_aFichesController[i].getPrevBet().toFixed(CURRENCY_DECIMAL));
            if (iValue > 0) {
                iTotBet += iValue;
