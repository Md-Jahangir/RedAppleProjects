...
function randomFloatBetween(minValue, maxValue, precision) {
    ...
    return parseFloat(Math.min(minValue + (Math.random() * (maxValue - minValue)), maxValue).toFixed(CURRENCY_DECIMAL));
}
...
function formatValue(iValue) {
    return Number.parseFloat(iValue).toFixed(CURRENCY_DECIMAL) + languageService.getCurrencySymbol()
}
...
function roundDecimal(num) {
    var decimal = Math.pow(10, CURRENCY_DECIMAL);
