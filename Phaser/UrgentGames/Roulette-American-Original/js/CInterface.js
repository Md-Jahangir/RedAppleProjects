function CInterface() {
    ...
    this._init = function() {
        ...
        var oMoneyText = new createjs.Text(languageService.getString("TEXT_MONEY"), "16px " + FONT1, "#fff");
        oMoneyText.textAlign = "center";
        oMoneyText.x = 272;
        oMoneyText.y = 105;
        s_oStage.addChild(oMoneyText);

        _oMoneyAmountText = new createjs.Text("0000000000", "16px " + FONT1, "#fff");
        ...
        var oCurBetText = new createjs.Text(languageService.getString("TEXT_CUR_BET"), "16px " + FONT1, "#fff");
        ...
        _oMsgTitle = new createjs.Text(languageService.getString("TEXT_MIN_BET") + ": " + serverRequests.getCurrencies().currencyBetMultiplier.BetMultiplierRoulette[0] + "\n" + languageService.getString("TEXT_MAX_BET") + ": " + serverRequests.getCurrencies().currencyBetMultiplier.BetMultiplierRoulette[1], "16px " + FONT1, "#fff");
        ...
        _oSpinBut = new CTextButton(1107, 641, s_oSpriteLibrary.getSprite('spin_but'), "  " + languageService.getString("TEXT_SPIN"), FONT1, "#fff", 19, s_oStage);
        _oSpinBut.setVisible(false);
        _oSpinBut.setAlign("left");
        _oSpinBut.addEventListener(ON_MOUSE_UP, this._onSpin, this);

        _oBetNeighbors = new CTextButton(350, 641, s_oSpriteLibrary.getSprite('but_bg'), languageService.getString("TEXT_NEIGHBORS"), FONT1, "#fff", 16);
        _oBetNeighbors.addEventListener(ON_MOUSE_UP, this._onNeighborsPanel, this);

        _oBetFinalsBet = new CTextButton(508, 641, s_oSpriteLibrary.getSprite('but_bg'), languageService.getString("TEXT_FINALSBET"), FONT1, "#fff", 16);
    ...
    this.setMoney = function(iMoney) {
        _oMoneyAmountText.text = parseFloat(iMoney).toFixed(languageService.serverCurrency('currencyDecimils')) + languageService.serverCurrency("currencySymbol");
        iStartMoney = parseInt(_oMoneyAmountText.text);
    };

    this.setCurBet = function(iCurBet) {
        _oBetAmountText.text = parseFloat(iCurBet).toFixed(languageService.serverCurrency('currencyDecimils')) + languageService.serverCurrency("currencySymbol");
    };

    this.setCurMaxBet = function(iCurMaxBet) {
        _oMsgTitle.text = languageService.getString("TEXT_MIN_BET") + ": " + serverRequests.getCurrencies().currencyBetMultiplier.BetMultiplierBlackjack[0] + "\n" + languageService.getString("TEXT_MAX_BET") + ": " + serverRequests.getCurrencies().currencyBetMultiplier.BetMultiplierBlackjack[1]
    };

    this.refreshMoney = function(iStartMoney, iMoney) {
        iStartMoney = parseFloat(_oMoneyAmountText.text);
