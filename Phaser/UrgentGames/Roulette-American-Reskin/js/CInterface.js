function CInterface() {
    ...
    this._init = function() {
        ...
        _pStartPosSettings = { x: 1707, y: 135 };
        ...
        var oMoneyText = new createjs.Text(languageService.getString("TEXT_MONEY"), "20px " + FONT_REGULAR, "#fff");
        ...
        var oCurBetText = new createjs.Text(languageService.getString("TEXT_CUR_BET"), "20px " + FONT_REGULAR, "#fff");
        ...
        //MINIMUM BET
        var oMinBetText = new createjs.Text(languageService.getString("TEXT_MIN_BET"), "20px " + FONT_REGULAR, "#fff");
        oMinBetText.textAlign = "left";
        oMinBetText.x = 850;
        oMinBetText.y = 105;
        s_oStage.addChild(oMinBetText);

        var oMinAmountText = new createjs.Text(languageService.serverCurrency("currencySymbol") + MIN_BET, "33px " + FONT_REGULAR, "#fff");
        ...
        //MAXIMUM BET
        var oMaxBetText = new createjs.Text(languageService.getString("TEXT_MAX_BET"), "20px " + FONT_REGULAR, "#fff");
        oMaxBetText.textAlign = "left";
        oMaxBetText.x = 970;
        oMaxBetText.y = 105;
        s_oStage.addChild(oMaxBetText);

        _oMaxAmountText = new createjs.Text(languageService.serverCurrency("currencySymbol") + MAX_BET, "33px " + FONT_REGULAR, "#fff");
        ...
        _oSpinBut = new CTextButton(1600, 960, s_oSpriteLibrary.getSprite('but_play_bg'), "  " + languageService.getString("TEXT_SPIN"), FONT_REGULAR, "#001755", 28, s_oStage);
        _oSpinBut.setVisible(false);
        _oSpinBut.addEventListener(ON_MOUSE_UP, this._onSpin, this);

        _oBetNeighbors = new CTextButton(895, 960, s_oSpriteLibrary.getSprite('but_neighbor_bg'), languageService.getString("TEXT_NEIGHBORS"), FONT_REGULAR, "#fff", 20);
        _oBetNeighbors.addEventListener(ON_MOUSE_UP, this._onNeighborsPanel, this);

        _oBetFinalsBet = new CTextButton(1100, 960, s_oSpriteLibrary.getSprite('but_final_bg'), languageService.getString("TEXT_FINALSBET"), FONT_REGULAR, "#fff", 20);
    ...
    this.setMoney = function(iMoney) {
        _oMoneyAmountText.text = languageService.serverCurrency("currencySymbol") + parseFloat(iMoney).toFixed(languageService.serverCurrency('currencyDecimils'));
    };

    this.setCurBet = function(iCurBet) {
        _oBetAmountText.text = languageService.serverCurrency("currencySymbol") + parseFloat(iCurBet).toFixed(languageService.serverCurrency('currencyDecimils'));
    };

    this.setCurMaxBet = function(iCurMaxBet) {
        _oMaxAmountText.text = languageService.serverCurrency("currencySymbol") + parseFloat(iCurMaxBet).toFixed(languageService.serverCurrency('currencyDecimils'));
    };

    this.refreshMoney = function(iStartMoney, iMoney) {
        _oRollingText = new CRollingTextController(_oMoneyAmountText, null, iStartMoney, parseFloat(iMoney), 1000, EASE_LINEAR, languageService.serverCurrency("currencySymbol"));
    };
