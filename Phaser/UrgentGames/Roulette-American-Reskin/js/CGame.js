function CGame(oData) {
    ...
    var _oGameOverPanel;
    var _sid, _bettedFiches = [];
    var _iUserMoney;
    var _iUserWin;
    var waitForServeResponse = false;
    ...
    this._init = function() {
        ...
        _oBg = createBitmap(s_oSpriteLibrary.getSprite('bg_gameplay'));
        ...
        _oFinalBet = new CFinalBetPanel(1465, 730);
    ...
    this._rouletteAnimEnded = function() {
        ...
        if (_iNumberExtracted === 0) {
            _oPlaceHolder.x = _oTableController.getEnlightX(_iNumberExtracted) + 25;
    ...
    this.updateBettedFichesEmpty = function() {
        _bettedFiches = [];
    }
    this.updateBettedFichesPop = function() {
        _bettedFiches.pop();
    }
    this.updateBettedFiches = function(obj) {
        _bettedFiches.push(obj);
    }
    ...
    this.onSpin = function() {
        if (waitForServeResponse) {
            return;
        }

        var aNumbersBetted = _oMySeat.getNumbersBetted();
        var aTmpNumbers = _oMySeat.getNumberSelected();
        var cred = _oMySeat.getCredit();
        var currBet = _oMySeat.getCurBet();

        if (mode === 'offline') {} else {
            var response = serverRequests.deductBetAmount(_oMySeat.getCurBet());
            _oMySeat.setCredit(response);
            _oInterface.setMoney(_oMySeat.getCredit());
        }

        var thisRef = this;
        waitForServeResponse = true;
        var msg = serverRequests.gameWinLoss(_sid, JSON.stringify(_bettedFiches), JSON.stringify(aTmpNumbers), function(_response) {
            console.log("_response: ", _response);
            if (_response) {
                waitForServeResponse = false;
            }
        }, this);
        _bettedFiches = [];
        try {
            var obj = JSON.parse(msg);
            _iNumberExtracted = obj.iNumberExtracted;
            _iCasinoCash = obj.casino_cash;
            _iUserMoney = obj.credit - obj.iWin;
            _iUserWin = obj.iWin;
            MAX_BET = obj.max_bet;
            console.log("MAX_BET: " + MAX_BET);
            console.log("_iNumberExtracted: " + _iNumberExtracted);
            _oInterface.setCurMaxBet(MAX_BET);
            thisRef._onServerWinLoss();
        } catch (e) {
            if (msg == 'lessBet') {} else if (msg == 'maxBet') {}
        }
    };

    this._onServerWinLoss = function() {
    ...
    console.log("init MIN_BET: " + MIN_BET);
    console.log("init MAX_BET: " + MAX_BET);
