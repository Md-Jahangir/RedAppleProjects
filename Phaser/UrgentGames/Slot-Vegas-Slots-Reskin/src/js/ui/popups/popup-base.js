...
console.log("id ",id);
	...
	create() {
		this.overlay = this.scene.add.image(0, 0, "popup-overlay").setOrigin(0);
	...
	createControlButtons() {
		if (this.config.hasCloseButton) {
			...
			this.closeButton.normal.visible=false;
			this.closeButton.hover.visible=false;
			this.closeButton.disabled.visible=false;
		}
	...
	resize(newWidth, newHeight) {
		...
		if (this.closeButton) {
			...
			if (this.config.closeButton) {
				console.log("close button taking from config");
		...
		if (this.acceptButton) {
			let x = this.width - this.baseSize * 7;
			let y = this.height - this.baseSize * 6.5;
