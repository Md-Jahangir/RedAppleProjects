...
class ValueSwitcher {
	...
	create() {
		...
		this.valueText = this.scene.add.text(
			...
			this.values[this.currentIndex],
			{
				...
				color: this.config.fontColor || "#fff9cb"//"#ffffff"
			}
		).setOrigin(0.5);
		...
		if (this.title) {
			this.titleText = this.scene.add.text(
				...
				this.title,
				{
					...
					color: this.config.title.fontColor || "#f4dc33",
					shadow: {
						offsetX: this.config.title.shadowOffsetX,
						offsetY: this.config.title.shadowOffsetY,
						color: "#000000",
						fill: true
					},
				},
	...
	setPosition(newX, newY) {
		...
		this.decrease.setPosition(this.x - width / 2, this.y + this.config.plusMinusButton.y);
		this.increase.setPosition(this.x + width / 2, this.y + this.config.plusMinusButton.y);
