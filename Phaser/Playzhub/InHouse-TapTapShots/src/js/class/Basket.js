...
export default class Basket {
    ...
    create() {
        ...
        this.timer = 10000;
        this.timerBase = this.scene.add.image(0, 0, 'timer-base').setDepth(3).setOrigin(0, 0.5);
        this.timerBody = this.scene.add.nineslice(0, 0, 'timer-bar', 0, 587, 6, 6).setDepth(3).setOrigin(0, 0.5);
        // console.log(this.timerBody, 'nine slice');
        this.scoreBase = this.scene.add.image(0, 0, 'score-base').setDepth(3);
        this.scoreText = this.scene.add.text(0, 0, this.score, { fontFamily: "Poppins-Bold", fontSize: 40 }).setOrigin(0.5).setDepth(3);
        this.RightBasket();
        this.LeftBasket();
    }
    //#region - Right Basket Create
    RightBasket() {
    ...
    //#region - Right Basket Score Checker Add
    ...
    //#region - Right Basket Visible Control
    ...
    //#region - Left Basket Score Checker Add
    ...
    //#region - Left Basket Visible Control
    LeftBasketVisibleOn() {
    ...
    //#region - Score Text Update
    UpdateScoreText() {
        Constant.score = this.score;
        this.scoreText.setText(this.score);
        this.scene.adHitFrom = 'level_complete'
        // setTimeout(async () => {
        //     await this.scene.IncrementLevelAndShowAd();
        // }, 500);
    }
    //#endregion

    //#region - Timer UI Control
    TimerControl() {
        switch (this.score) {
            case 1:
                this.TimerTween();
            ...
            default:
                ...
                // this.scene.matter.world.engine.gravity.y = window.innerHeight / 349.09;
    ...
    TimerTween() {
        ...
        this.gameTimer.on('complete', () => {
            ...
            // this.scene.matter.world.engine.gravity.y = window.innerHeight / 1920;
    ...
    //#region - Timer Tween Restart Control
    TimerTweenRestartAfterScore() {
        ...
        this.timerBody.width = 583;
        // console.log('restart tween');
        this.TimerTween();
    }
    //#endregion

    //#region - Which basket visible that functionality control
    ...
    //#region - Show & Hide Baskets functionality
    ShowHideBasket() {
    ...
    resize(newWidth, newHeight, newScale) {
        ...
        this.timerBody.setPosition(this.timerBase.x + 8 * newScale, this.timerBase.y);

        this.scoreBase.setScale(newScale);
        this.scoreBase.setPosition((newWidth / 2) - (0 * newScale), 100 * newScale);
        this.scoreText.setScale(newScale);
        this.scoreText.setPosition(this.scoreBase.x + (10 * newScale), this.scoreBase.y - (5 * newScale));
