...
export default class DragRope {
    constructor(scene, _dragRopeConfig, _index) {
        this.scene = scene;
        this.index = _index;
    ...
    create() {
        ...
        for (let index = 0; index < this.dragRopeConfig.dragRopebodyCount; index++) {
            let body = this.scene.matter.add.circle(Constant.clientWidth / 2 + this.dragRopeConfig.dragRopeHeadPosition.x[this.index] * newScale, Constant.clientHeight / 2 + this.dragRopeConfig.dragRopeHeadPosition.y[this.index] * newScale, 1 * newScale, { isStatic: false, label: 'body' + index, isSensor: true });
