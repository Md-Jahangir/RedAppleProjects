...
export default class TweenRunTimeRope {
    constructor(scene, bodyCount, isRopeEnemy, _tweenConfig, _index, areaRadious) {
        this.scene = scene;
    ...
    create() {
        ...
        setTimeout(() => {
            this.RuntimeRopeTweenControl(this.tweenConfig.direction, this.tweenConfig.time, this.tweenConfig.endPos);
    ...
    RuntimeCreate() {
        ...
        for (let index = 0; index < this.bodyCount; index++) {
            let body = this.scene.matter.add.circle((this.scene.candy.x + this.ropeHead1.x) / 2, (this.scene.candy.y + this.ropeHead1.y) / 2, 1 * this.newScale, { isStatic: false, label: 'body' + index });
        ...
        this.constraintsArray.push(this.scene.matter.add.constraint(this.ropeHead1, this.bodyArray[0], 90 * this.newScale, 1.4, /*{ pointA: { x: 0, y: 65 } }*/));

        for (let i = 0; i < this.bodyArray.length - 1; i++) {
            let constraint = this.scene.matter.add.constraint(this.bodyArray[i], this.bodyArray[i + 1], 20 * this.newScale, 1.5, /*{ pointA: { x: 0, y: 0 } }*/);
            // constraint.damping = 0.2
            this.constraintsArray.push(constraint);
        }
        this.constraintsArray.push(this.scene.matter.add.constraint(this.bodyArray[this.bodyArray.length - 1], this.scene.candy, 90 * this.newScale, 1.4, /*{ pointA: { x: 0, y: 0 } }*/));
    ...
    RuntimeRopeTweenControl(_direction, _time, _endPos) {
        console.log('tweenData', _direction, _time, _endPos);
        this.runTimeRopeTween = this.scene.tweens.add({
            targets: [this.ropeArea, this.ropeHead1, this.ropeHead2],  // Array of objects as the targets
    ...
    RunTimeRopeTweenRemove() {
        this.runTimeRopeTween.stop();
        this.runTimeRopeTween.remove();
        this.runTimeRopeTween = null;
    }
