export default class GameScene extends Phaser.Scene {

    constructor() {
        super("GameScene");
        this.soundButton;
        this.ball;
        this.tapTosStart;
        this.gameplayContainer = null;
        this.isGameStart = false;
        this.group1;
        this.batArry = [];
        this.randomNumber = [];
        // roundCounter = 2;
        this.tapCounter = -1;
        this.numberArray1 = [];
        this.isWicket = false;
        this.defaultPosX = [];
        this.defaultPosY = [];
        this.batArrayTweenPosX = [];
        this.batArrayTweenPosY = [];
        this.isGameOver = false;
        this.midBlock;
        this.highScore = 0;
        this.gamePlaySceneScoreTxt;
        this.roundCounterTxt;
        this.isCollision = false;
        this.animationTime = 250;
        this.isGamePaused = false;
    }
    ResetAllData() {
        this.gameplayContainer = null;
        this.isGameStart = false;
        this.batArry = [];
        this.batArrayForScore = [];
        this.randomNumber = [];
        // this.roundCounter = 2;
        this.tapCounter = -1;
        this.numberArray1 = [0, 1, 2, 3, 4, 5, 6, 7, 8];
        this.isWicket = false;
        this.defaultPosX = [];
        this.defaultPosY = [];
        this.batArrayTweenPosX = [];
        this.batArrayTweenPosY = [];
        this.isGameOver = false;
        // score = 0;
        this.highScore = 0;
        this.animationTime = 250;
    }
    create(gamedata) {
        PlayzhubEventHandler.AdStarted(this.PauseGame.bind(this));
        PlayzhubEventHandler.AdCompleted(this.ResumeGame.bind(this));
        console.log('gamedata', gamedata);
        playClicked++;
        PlayzhubEventHandler.GamePlayStarted(playClicked);
        /// Update game-state and game-data inisitially if restarted the game again to retrieve the last level score 

        // if (firstTimeGameStarted == 0) {
        //     score = 0;
        //     roundCounter = 2;
        //     window.addEventListener("message", (event) => {
        //         if (event.data && event.data.eventName == "update-game-state") {
        //             const gamedata = event.data.data;
        //             if (gamedata == null) {
        //                 score = 0;
        //                 roundCounter = 2;
        //             } else {
        //                 score = gamedata.game_score;
        //                 roundCounter = gamedata.round;
        //             }
        //             console.log("Game srtate listened ", score)
        //         }
        //     })
        //     window.postMessage('game-ready');
        //     firstTimeGameStarted = 1
        // } else {
        //     window.addEventListener("message", (event) => {
        //         if (event.data && event.data.eventName == "response-game-state") {
        //             const gamedata = event.data.data;
        //             if (gamedata == null) {
        //                 score = 0;
        //                 roundCounter = 2;
        //             } else {
        //                 score = gamedata.game_score;
        //                 roundCounter = gamedata.round;
        //             }
        //             console.log("response-game-state from platform ", score)
        //         }
        //     })
        //     const eventName = 'request-game-state';
        //     const data = null;
        //     window.parent.postMessage({ eventName, data }, "*");
        // }


        score = gamedata.score;
        roundCounter = gamedata.round + 1;
        gameStartTime = Date.now();
        this.isGamePaused = false;
        this.ResetAllData();
        for (let i = 0; i < 26; i++) {
            this.numberArray1.push(i);
        }
        // for(let j = 32;j<35;j++)
        // {
        //     this.numberArray1.push(j);
        // }
        // console.log("Enter into the Gameplay Scene"+this.tapCounter);
        let gameplayBg = this.add.sprite(Math.round(game.config.width / 2), Math.round(game.config.height / 2), "backGround")
            .setOrigin(0.5, 0.5)
            .setScale(scaleFactor * 1.5, scaleFactor * 1.5)
            .setInteractive({ useHandCursor: true })
            .on('pointerdown', this.GameplayBgBttnPressed)

        this.tapTosStart = this.add.sprite(Math.round(game.config.width / 2), Math.round(game.config.height / 1.15), "tapToStart")
            .setOrigin(0.5, 0.5)
            .setScale(scaleFactor * 1.5, scaleFactor * 1.5)

        this.tweens.add({
            targets: [this.tapTosStart],
            alpha: 0,
            delay: 1000,
            duration: 300,
            ease: 'Power2',
            onComplete: () => {
                this.tweens.add({
                    targets: [this.playBttn],
                    alpha: 1,
                    duration: 300,
                    ease: 'Power2',
                }, this);
            },
            yoyo: true,
            loop: 1000
        }, this);


        let roundBg = this.add.sprite(Math.round(game.config.width / 2), Math.round(game.config.height / 10), "roundBg")
            .setOrigin(0.5, 0.5)
            .setScale(scaleFactor * 1.5, scaleFactor * 1.5)

        let roundTxt = this.add.sprite(Math.round(game.config.width / 2.5), Math.round(game.config.height / 10), "roundTxt")
            .setOrigin(0.5, 0.5)
            .setScale(scaleFactor, scaleFactor)

        Utils.SpriteSettingsControl(this, game.config.width / 2, game.config.height / 2.2, "circle", 0.5, 0.5, scaleFactor * 1.4, scaleFactor * 1.4);

        this.soundButton = Utils.SpriteSettingsControl(this, Math.round(game.config.width / 1.1), Math.round(game.config.height / 24),
            "soundOn", 0.5, 0.5, scaleFactor * 2, scaleFactor * 2, "true", this.SoundButtonPressed, this.SoundButtonReleased);
        this.DefaultSoundButton();

        let homeBttn = Utils.SpriteSettingsControl(this, Math.round(game.config.width / 10.5), Math.round(game.config.height / 24), "back",
            0.5, 0.5, scaleFactor * 2, scaleFactor * 2, "true", this.HomeBttnPressed, this.HomeBttnReleased);

        this.midBlock = this.add.sprite(Math.round(game.config.width / 1.8), Math.round(game.config.height / 4.3), "Wicket0")
            .setOrigin(0.5, 0.5)
            .setScale(scaleFactor, scaleFactor);
        this.midBlock.visible = false;

        this.anims.create({
            key: 'wicketAnim',
            frames: [
                { key: 'Wicket0' },
                { key: 'Wicket1' },
                { key: 'Wicket2' },
                { key: 'Wicket3' },
                { key: 'Wicket4' },
                { key: 'Wicket0', duration: 2 },
            ],
            frameRate: 10,
            repeat: 0
        });

        this.ball = this.add.sprite(Math.round(game.config.width / 35), Math.round(game.config.height / 5.2), "ball")
            .setOrigin(0.5, 0.5)
            .setScale(scaleFactor * 2, scaleFactor * 2);

        this.gameplayContainer = this.add.container(Math.round(game.config.width / 2), Math.round(game.config.height / 2.2));
        this.gameplayContainer.setScale(scaleFactor * 0.5, scaleFactor * 0.5);
        this.gameplayContainer.add([this.ball]);

        this.ball.y -= 1100;
        // console.log("The Position Set.....................",this.ball.y);

        this.group1 = this.add.group({ key: 'bat0', frameQuantity: 36 });
        const circle = new Phaser.Geom.Circle(Math.round(game.config.width / 2), Math.round(game.config.height / 2.2), 380);
        Phaser.Actions.PlaceOnCircle(this.group1.getChildren(), circle);

        this.group2 = this.add.group({ key: 'one_pixel_white', frameQuantity: 36 });
        const circle1 = new Phaser.Geom.Circle(Math.round(game.config.width / 2), Math.round(game.config.height / 2.2), 560);
        Phaser.Actions.PlaceOnCircle(this.group2.getChildren(), circle1);


        for (let i = 26; i < 31; i++) {
            this.group1.getChildren()[i].visible = false;
        }

        // this.group1.getChildren()[0].angle = 90;
        // this.group1.getChildren()[1].angle = 90+30;
        // this.group1.getChildren()[2].angle = 90+60;
        // this.group1.getChildren()[3].angle = 180;

        // this.group1.getChildren()[4].angle = 180+30;
        // this.group1.getChildren()[5].angle = 180+60;
        // this.group1.getChildren()[6].angle = 270;

        // this.group1.getChildren()[7].angle = 270+30;
        // this.group1.getChildren()[8].angle = 270+60;

        // this.group1.getChildren()[11].angle = 360+60;


        // this.group1.getChildren()[10].angle = 360+30;
        // this.group1.getChildren()[12].angle = 360+60;


        for (let i = 0; i < this.group1.getChildren().length; i++) {
            // console.log("The Posx................."+parseInt(this.group1.getChildren()[i].x));
            // console.log("The Posy................."+parseInt(this.group1.getChildren()[i].y));

            this.defaultPosX.push(parseInt(this.group1.getChildren()[i].x));
            this.defaultPosY.push(parseInt(this.group1.getChildren()[i].y));

            this.batArrayTweenPosX.push(parseInt(this.group2.getChildren()[i].x));
            this.batArrayTweenPosY.push(parseInt(this.group2.getChildren()[i].y));
            // this.group2.getChildren()[i].setScale(15,15);

            // this.group1.getChildren()[i].setScale(0,0);
            this.group1.getChildren()[i].angle = 90 + (i * 10);
            this.batArry.push(this.add.sprite(this.group1.getChildren()[i].x, this.group1.getChildren()[i].y, "one_pixel_white")
                .setOrigin(0.5, 0.5)
                .setScale(scaleFactor * 15, scaleFactor * 15));
            this.batArry[i].alpha = 0.001;


            this.batArrayForScore.push(this.add.sprite(this.group1.getChildren()[i].x, this.group1.getChildren()[i].y, "one_pixel_white")
                .setOrigin(0.5, 0.5)
                .setScale(scaleFactor * 15, scaleFactor * 15));
            this.batArrayForScore[i].alpha = 0.001;

        }
        // console.log("The bat Array...............",this.batArry.length);

        for (let i = 26; i < 36; i++) {
            this.batArry[i].setScale(0, 0);
        }
        // this.batArry[9].setScale(0,0);
        // this.batArry[10].setScale(0,0);


        let titleSceneTxtStyle = { fontFamily: 'burgerjointneon', fontSize: '85px', fill: '#ffffff', fontStyle: 'bold', align: 'center' };
        this.gamePlaySceneScoreTxt = this.add.text(Math.round(game.config.width / 2), Math.round(game.config.height / 2.2),
            score, titleSceneTxtStyle)
            .setOrigin(0.5, 0.5)
            .setScale(scaleFactor, scaleFactor);

        this.roundCounterTxt = this.add.text(Math.round(game.config.width / 1.5), Math.round(game.config.height / 9.5),
            (roundCounter), titleSceneTxtStyle)
            .setOrigin(0.5, 0.5)
            .setScale(scaleFactor, scaleFactor);


        // let emitterPos1 = this.add.sprite(Math.round(game.config.width/4), Math.round(game.config.height/4), "one_pixel_white")
        //     .setOrigin(0.5, 0.5)
        //     .setScale(scaleFactor*30, scaleFactor*30);

        // let emitterPos2 = this.add.sprite(Math.round(game.config.width/1.2), Math.round(game.config.height/4), "one_pixel_white")
        // .setOrigin(0.5, 0.5)
        // .setScale(scaleFactor*30, scaleFactor*30);
        this.CreateQuitPopup();
        this.SetAllBatScaleToZero();

        GameAnalytics("addDesignEvent", "screen:gameplay");

        GameAnalytics("addProgressionEvent", "Start", "hit_the_wicket_endless");
    }

    PauseGame() {
        console.log(' Game paused ');
        GameAnalytics("addDesignEvent", "ad:started");
        // PlayzhubEventHandler.GamePlayPaused();
    }

    ResumeGame() {
        console.log(' Game Resumed ');
        GameAnalytics("addDesignEvent", "ad:completed");
        // SoundManager.PlayMenuPageBgMusic();
        // PlayzhubEventHandler.GamePlayResumed();
    }
    CreateQuitPopup() {

        this.blackOverlay = this.add.sprite(Math.round(game.config.width / 2), Math.round(game.config.height / 2), "one_pixel_white")
            .setOrigin(0.5, 0.5).setScale(scaleFactor * 1080, scaleFactor * 1920).setVisible(false).setInteractive().setAlpha(0.05);
        this.quitPopupContainer = this.add.container(0, 0);
        this.quitBg = this.add.sprite(Math.round(game.config.width / 2), Math.round(game.config.height / 2), "quit_bg")
            .setOrigin(0.5, 0.5)
            .setScale(scaleFactor * 1.5, scaleFactor * 1.5);

        this.quitYes = this.add.sprite(Math.round(game.config.width / 3), Math.round(game.config.height / 1.45), "quit_yes")
            .setOrigin(0.5, 0.5).setScale(scaleFactor * 1.5, scaleFactor * 1.5).setInteractive({ useHandCursor: true });

        this.quitNo = this.add.sprite(Math.round(game.config.width / 1.5), Math.round(game.config.height / 1.45), "quit_no")
            .setOrigin(0.5, 0.5).setScale(scaleFactor * 1.5, scaleFactor * 1.5).setInteractive({ useHandCursor: true });
        this.quitYes.on('pointerup', this.onQuitYesButtonRelease, this);
        this.quitNo.on('pointerup', this.onQuitNoButtonRelease, this);
        this.quitPopupContainer.add([this.quitBg, this.quitNo, this.quitYes]);
        this.quitPopupContainer.setVisible(false);
    }
    onQuitYesButtonRelease() {
        const currentTimeStamp = Date.now();
        const finalTime = currentTimeStamp - gameStartTime;
        // Server.PostGameQuitToParent(finalTime / 1000, score);
        PlayzhubEventHandler.GamePlayStopped(finalTime / 1000);
        this.isGamePaused = false;
        this.blackOverlay.setVisible(false);
        this.quitPopupContainer.setVisible(false);
        // SoundManager.StopGamePlayPageBgMusic();
        // SoundManager.PlayMenuPageBgMusic();
        this.scene.stop("GameScene");
        this.scene.start("TitleScene");

    }
    onQuitNoButtonRelease() {
        this.isGamePaused = false;
        this.blackOverlay.setVisible(false);
        this.quitPopupContainer.setVisible(false);
        SoundManager.PlayGameplayPageBgMusic();
    }
    InstantiateBallWithRoundCounter(_roundCounter) {
        this.GetRandomNumber(_roundCounter, this.numberArray1);
    }
    checkOverlap(spriteA, spriteB) {
        let boundsA = spriteA.getBounds();
        let boundsB = spriteB.getBounds();
        return Phaser.Geom.Intersects.RectangleToRectangle(boundsA, boundsB);
    }
    update() {
        if (this.isGamePaused) return;
        this.ball.angle += 6;
        if (this.isGameStart) {
            this.gameplayContainer.angle += (1 + roundCounter / 10);
        }
        if (!this.isGameOver) {
            for (let i = 0; i < this.randomNumber.length; i++) {
                this.CheckCollision(this.randomNumber[i]);
            }
            if (this.checkOverlap(this.ball, this.batArry[27]) && !this.isWicket) {
                this.isWicket = true;
                SoundManager.BallHitWicketPlayCheerSound();
                // console.log("Wicket Happens Next Round Start");
                this.NextRound();
            }
        }
    }
    NextRound() {
        score += 10;
        const currentTimeStamp = Date.now();
        const finalTime = currentTimeStamp - gameStartTime;
        PlayzhubEventHandler.GameScoreUpdate(finalTime / 1000, score);
        PlayzhubEventHandler.UpdateGameLevel(finalTime / 1000, roundCounter, 'complete');
        PlayzhubEventHandler.InterimBreak();
        GameAnalytics("addProgressionEvent", "Complete", "hit_the_wicket_endless", undefined, undefined, score);
        GameAnalytics("addDesignEvent", "score:hit_the_wicket", score);

        const stateData = {
            'game_score': score,
            'round': roundCounter
        }
        PlayzhubEventHandler.GameStateUpdate(stateData);
        gameStartTime = Date.now();
        this.midBlock.play('wicketAnim');
        this.gamePlaySceneScoreTxt.text = score;
        roundCounter++;
        this.roundCounterTxt.text = (roundCounter);
        this.randomNumber = [];
        this.SetAllBatScaleToZero();
        this.tapCounter = 0;

        for (let i = 0; i < 50; i++) {
            let emitter0 = this.add.particles('spark').createEmitter({
                x: Math.round(game.config.width / 4),
                y: Math.round(game.config.height / 4),
                speed: { min: -200, max: 200 },
                angle: { min: 0, max: 360 },
                scale: { start: 0.2, end: 0 },
                blendMode: 'ADD',
                lifespan: 1000,
                gravityY: 200
            });
            emitter0.explode();
        }

        for (let i = 0; i < 50; i++) {
            let emitter0 = this.add.particles('spark').createEmitter({
                x: Math.round(game.config.width / 2),
                y: Math.round(game.config.height / 4),
                speed: { min: -200, max: 200 },
                angle: { min: 0, max: 360 },
                scale: { start: 0.2, end: 0 },
                blendMode: 'ADD',
                lifespan: 1000,
                gravityY: 200
            });
            emitter0.explode();
        }


        for (let i = 0; i < 50; i++) {
            let emitter0 = this.add.particles('spark').createEmitter({
                x: Math.round(game.config.width / 1.5),
                y: Math.round(game.config.height / 4),
                speed: { min: -200, max: 200 },
                angle: { min: 0, max: 360 },
                scale: { start: 0.2, end: 0 },
                blendMode: 'ADD',
                lifespan: 1000,
                gravityY: 200
            });
            emitter0.explode();
        }

        setTimeout(() => {
            this.isWicket = false;
            this.InstantiateBallWithRoundCounter(roundCounter);
        }, 300);
    }
    SetAllBatScaleToZero() {
        for (let i = 0; i < this.group1.getChildren().length; i++) {
            this.group1.getChildren()[i].setScale(0, 0);
        }
    }
    GetRandomNumber(_counter, _arrayElement1) {
        for (let i = 0; i < _counter; i++) {
            let rand1 = _arrayElement1[Math.floor(Math.random() * _arrayElement1.length)];
            for (let j = 0; j < this.randomNumber.length; j++) {
                while (rand1 == this.randomNumber[j]) {
                    // console.log("Enter into the Rand1......................"+rand1);
                    rand1 = _arrayElement1[Math.floor(Math.random() * _arrayElement1.length)];
                }
            }
            // console.log("The rand 1................."+rand1);
            let rand2 = Math.floor(Math.random() * 2);
            if (rand2 == 0)
                this.group1.getChildren()[rand1].setTexture("bat0");
            else
                this.group1.getChildren()[rand1].setTexture("bat1");
            this.group1.getChildren()[rand1].setScale(scaleFactor * 1.2, scaleFactor * 1.2);
            this.randomNumber.push(rand1);
        }
        // for(let k = 0;k<this.randomNumber.length;k++)
        // {
        //     if(this.randomNumber[k] == 11 || this.randomNumber[k] == 12)
        //     {
        //         this.randomNumber.reverse();
        //     }
        //     else{
        //     }
        // }
        this.randomNumber.sort(function (a, b) { return a - b });
        // console.log("Random Number Push.............",this.randomNumber);
    }
    BatMovement(_randomNumberArray, _counter) {
        // console.log("The counter.............."+_counter+"The round counter.................."+this.animationTime);
        if (_counter == (roundCounter - 1)) {
            this.batArry[27].setScale(15, 15);
        }
        if (!this.isGameOver && _counter <= (roundCounter - 1)) {
            SoundManager.PlaySwingSound();
            this.tweens.add({
                targets: [this.group1.getChildren()[_randomNumberArray[_counter]]],
                y: { value: this.batArrayTweenPosY[_randomNumberArray[_counter]] },
                x: { value: this.batArrayTweenPosX[_randomNumberArray[_counter]] },
                delay: 0,
                duration: this.animationTime,
                ease: "Linear",
                onComplete: () => {
                    // console.log("Animation Complete...................");
                    this.tweens.add({
                        targets: [this.group1.getChildren()[_randomNumberArray[_counter]]],
                        y: { value: this.defaultPosY[_randomNumberArray[_counter]] },
                        x: { value: this.defaultPosX[_randomNumberArray[_counter]] },
                        delay: 0,
                        duration: this.animationTime,
                        ease: "Linear",
                    }, this);
                },
            }, this);

            this.tweens.add({
                targets: [this.batArry[_randomNumberArray[_counter]]],
                y: { value: this.batArrayTweenPosY[_randomNumberArray[_counter]] },
                x: { value: this.batArrayTweenPosX[_randomNumberArray[_counter]] },
                delay: 0,
                duration: this.animationTime,
                ease: "Linear",
                onComplete: () => {
                    // console.log("Animation Complete...................");
                    // score++;
                    // this.gamePlaySceneScoreTxt.text = score;
                    this.tweens.add({
                        targets: [this.batArry[_randomNumberArray[_counter]]],
                        y: { value: this.defaultPosY[_randomNumberArray[_counter]] },
                        x: { value: this.defaultPosX[_randomNumberArray[_counter]] },
                        delay: 0,
                        duration: this.animationTime,
                        ease: "Linear",
                    }, this);
                },
            }, this);
        }
    }
    CheckCollision(_index) {

        if (this.checkOverlap(this.ball, this.batArrayForScore[_index + 1])) {
            if (!this.isCollision) {
                this.isCollision = true;
                // console.log("The score increases...............");
                SoundManager.BallPassesBatPlayCheerSound();
                score++;
                this.gamePlaySceneScoreTxt.text = score;
                let randNumber = Math.floor(Math.random() * (4 - 2) + 2);
                for (let i = 0; i < 50; i++) {
                    let emitter0 = this.add.particles('spark').createEmitter({
                        x: Math.round(game.config.width / randNumber),
                        y: Math.round(game.config.height / 4),
                        speed: { min: -200, max: 200 },
                        angle: { min: 0, max: 360 },
                        scale: { start: 0.2, end: 0 },
                        blendMode: 'ADD',
                        lifespan: 1000,
                        gravityY: 0
                    });
                    emitter0.explode();
                }
                setTimeout(() => {
                    this.isCollision = false;
                }, 300);
            }
        }
        if (this.checkOverlap(this.ball, this.batArry[_index])) {
            // console.log("Overlap happens................................");
            SoundManager.StopGamePlayPageBgMusic();
            SoundManager.StopMenuPageBgMusic();
            SoundManager.BallHitsBatPlayCheerSound();
            setTimeout(() => {
                GameAnalytics("addDesignEvent", "ad:requested");
                PlayzhubEventHandler.RequestAD();
            }, 300);
            this.isGameOver = true;
            this.isGameStart = false;
            setTimeout(() => {
                const currentTimeStamp = Date.now();
                const finalTime = currentTimeStamp - gameStartTime;
                PlayzhubEventHandler.UpdateGameLevel(finalTime / 1000, roundCounter, 'incomplete')

                // SoundManager.PlayMenuPageBgMusic();
                this.scene.stop("GameScene");
                this.scene.start("TitleScene");
                // this.scene.restart("GameScene");
                // localStorage.setItem("score", score);
            }, 1000);
        }
    }
    GameplayBgBttnPressed() {
        GameAnalytics("addDesignEvent", "ui:tap_to_start_clicked");
        if (this.scene.tapCounter == -1) {
            this.scene.tapCounter++;
            this.scene.tapTosStart.visible = false;
            this.scene.isGameStart = true;
            this.scene.InstantiateBallWithRoundCounter(roundCounter);
            this.scene.midBlock.visible = true;
        }
        else {
            this.scene.BatMovement(this.scene.randomNumber, this.scene.tapCounter);
            this.scene.tapCounter++;
        }
    }
    DefaultSoundButton() {
        if (localStorage.getItem("hit_the_wicket_is_sound_on") == null) {
            localStorage.setItem("hit_the_wicket_is_sound_on", 1);
        }
        if (localStorage.getItem("hit_the_wicket_is_sound_on") == 1) {
            this.soundButton.setTexture("soundOn");
        } else {
            this.soundButton.setTexture("soundOff");
        }
    }
    ToggleSoundButton() {
        if (localStorage.getItem("hit_the_wicket_is_sound_on") == 1) {
            localStorage.setItem("hit_the_wicket_is_sound_on", 0);
            this.soundButton.setTexture("soundOff");
            SoundManager.PlayButtonClickSound();
            SoundManager.StopGamePlayPageBgMusic();
        } else {
            localStorage.setItem("hit_the_wicket_is_sound_on", 1);
            this.soundButton.setTexture("soundOn");
            SoundManager.PlayGameplayPageBgMusic();
        }
    }
    SoundButtonPressed() {
        GameAnalytics("addDesignEvent", "ui: sound_clicked");
        Utils.ButtonScaleTween(this.scene.scene, this.scene.scene.soundButton, scaleFactor * 2);
        SoundManager.PlayButtonClickSound();
    }
    SoundButtonReleased() {
        this.ToggleSoundButton();
    }
    HomeBttnPressed() {
        GameAnalytics("addDesignEvent", "ui: back_clicked");
        GameAnalytics("addDesignEvent", "ad:requested");
        PlayzhubEventHandler.RequestAD();
        SoundManager.StopGamePlayPageBgMusic();
        // SoundManager.PlayMenuPageBgMusic();
        // this.scene.stop("GameScene");
        // this.scene.start("TitleScene");
        this.isGamePaused = true;
        this.blackOverlay.setVisible(true);
        this.quitPopupContainer.setVisible(true);
    }
    HomeBttnReleased() {

    }

}