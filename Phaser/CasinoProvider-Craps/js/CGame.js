function CGame(oData) {
    ...
    this._init = function () {
        ...
        _oMySeat = new CSeat();
        // _oPuck = new CPuck(257, 225, s_oStage);
        _oPuck = new CPuck(440, 160, s_oStage);//initial(dont come)
        // _oPuck = new CPuck(560, 160, s_oStage);//number 4
        // _oPuck = new CPuck(700, 160, s_oStage);//number 5
        // _oPuck = new CPuck(830, 160, s_oStage);//number 6
        // _oPuck = new CPuck(960, 160, s_oStage);//number 8
        // _oPuck = new CPuck(1090, 160, s_oStage);//number 9
        // _oPuck = new CPuck(1220, 160, s_oStage);//number 10


        _oInterface = new CInterface();

        _oDicesAnim = new CDicesAnim(0, 0);
    ...
    this._setState = function (iState) {
        ...
        switch (iState) {
            case STATE_GAME_WAITING_FOR_BET:
                {
                    // console.log("_oMySeat.getCredit(): ", _oMySeat.getCredit());
                    // console.log("s_oGameSettings.getFicheValues(0): ", s_oGameSettings.getFicheValues(0));
                    // if (_oMySeat.getCredit() < s_oGameSettings.getFicheValues(0)) {
                    //     _iState = -1;
                    //     setTimeout(function () {
                    //         _oInterface.hideBlock();
                    //         _oGameOverPanel.show();
                    //         console.log("000_oMySeat.getCredit(): ", _oMySeat.getCredit());
    ...
    this._generateWinLoss = function () {
        ...
        for (var szBet in _aBetHistory) {
            ...
            // console.log("oRet: ", oRet);
            // console.log("oRet: ", oRet.data.result.result);
            // aDiceForLose = oRet.lose;
            // aDiceForWin = oRet.win;
            if (serverRequests.mode == 'online') {
                aDiceForLose = oRet.data.result.result.lose;
                aDiceForWin = oRet.data.result.result.win;
            } else {
                aDiceForLose = oRet.lose;
                aDiceForWin = oRet.win;
            }
        ...
        if (iRand >= iWinOccurence) {
            ...
            if (_iContRolling > _iMaxNumRolling) {
                ...
                do {
                    if (serverRequests.mode === 'offline') {
                        ...
                        var response = serverRequests._generateRandomDices();
                        // var aDices = response.result;
                        var aDices = response.data.result.result;
                    }
            ...
            if (_iContRolling > _iMaxNumRolling) {
                ...
                if (szBet.indexOf("hardway") !== -1) {
                    if (serverRequests.mode === 'offline') {
                        this._checkHardwayWin(szBet);
                    } else {
                        serverRequests._checkHardwayWin(szBet);
                    }
                } else {
                    do {
                        if (serverRequests.mode === 'offline') {
    ...
    this._generateRandomDices = function () {
        ...
        let randomDicesResp = serverRequests._generateRandomDices();

        // return randomDicesResp.result;
        return randomDicesResp.data.result.result;
    }
    ...
    this._checkHardwayWin = function (szBet) {
        var aRandDicesHardwayWinResp = serverRequests._checkHardwayWin(szBet);
        do {
            var aDices = this._generateRandomDices();
        ...
        for (var szBet in _aBetHistory) {
            ...
            if (iWin !== -1) {
                ...
                if (iWin > 0) {
                    ...
                    new CScoreText(iWin + languageService.serverCurrency("currencySymbol"), pPosFiche.x, pPosFiche.y);
                }

            }
        }
        if (iTotWin > 0) {
            _oInterface.refreshMsgHelp(languageService.getString("TEXT_YOU_WIN") + ": " + iTotWin);
    ...
    this.onRoll = function () {
        ...
        if (_oMySeat.getCurBet() < MIN_BET) {
            _oMsgBox.show(languageService.getString("TEXT_ERROR_MIN_BET"));
            _oInterface.enableBetFiches();
            _oInterface.enableRoll(true);
            return;
        }
        ...
        if (serverRequests.mode === 'offline') { } else {
            _oInterface.setMoney(_oMySeat.getCredit());
        }
    ...
    this._onShowBetOnTable = function (oParams) {
        ...
        if ((_oMySeat.getCredit() - iFicheValue) < 0) {
            //SHOW MSG BOX
            _oMsgBox.show(languageService.getString("TEXT_ERROR_NO_MONEY_MSG"));
            return;
        }

        if ((iCurBet + iFicheValue) > MAX_BET) {
            _oMsgBox.show(languageService.getString("TEXT_ERROR_MAX_BET_REACHED"));
            return;
        }
        ...
        _oInterface.enableRoll(true);
        _oInterface.enableClearButton();
        _oInterface.refreshMsgHelp(languageService.getString("TEXT_READY_TO_ROLL"), true);
    ...
    this._onShowEnlight = function (oParams) {
        ...
        if (szEnlight) {
            ...
            _oInterface.refreshMsgHelp(languageService.getString("TEXT_HELP_MSG_" + szEnlight), false);
        }
    };
    ...
    s_oGame = this;
    console.log("oooo game oData: ", oData);
