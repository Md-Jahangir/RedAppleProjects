...
function sizeHandler() {
   ...
   var iAdd = 0;
    ...
    if (fOffsetX * fGameInverseScaling < -EDGEBOARD_X ||
        ...
        if(!s_bMobile || isIOS)
        {
            console.log('is crome'+isChrome()   );
            destW = CANVAS_WIDTH * multiplier + 20;
        }

        // destW = CANVAS_WIDTH * multiplier + 50;
...
function getStraightPoint(t, p) {
    var result = new createjs.Point();
    result.x = (p.end.x - p.start.x) * t + p.start.x;
    result.y = (p.end.y - p.start.y) * t + p.start.y;
    return result;
}
