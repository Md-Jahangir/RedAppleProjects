...
export default class BootScene extends Phaser.Scene {
    ...
    preload() {
        ...
        // this.load.plugin('rexgrayscalepipelineplugin', 'https://raw.githubusercontent.com/rexrainbow/phaser3-rex-notes/master/dist/rexgrayscalepipelineplugin.min.js');
    ...
    async onDataRecieved(_this) {
        if (Server.platform && Server.platform == "favorites") {
            ...
            if (data.error == 0) {
                ...
                localStorage.setItem("InMySkinJsonData", JSON.stringify(data.data));
    ...
    ImageData(_data) {
        localStorage.setItem("InMySkinImageJson", JSON.stringify(_data.data));
    }
    create() {
        ...
        if (checkFirstTime) {
            ...
            if (this.CheckInternetConnection())/*internet is connected*/ {
                ...
                let _data = JSON.parse(localStorage.getItem("InMySkinJsonData"));
