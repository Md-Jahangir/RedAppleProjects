...
class Database {
    ...
    ParseAndStore(_json) {
        ...
        //==========Storing JSON DATA ================//
        this.average_answer_time = jsonData.specific.average_answer_time; // jsonData.average_answer_time;
    ...
    {
        switch (_param) {
            case "factor": return this.factor;
            case "bg_image_location": return this.bg_image_location;
            case "main_image_location": return this.main_image_location;
            case "color_scheme": return this.color_scheme
            case "answer_location": return this.answer_location;
            case "answer_location_spread": return this.answer_location_spread;
            case "answer_location_spread_value": return this.answer_location_spread_value;
            case "movement_direction": return this.movement_direction;
            case "rotation": return this.rotation;
            case "rotation_time": return this.rotation_time;
            case "level_down": return this.level_down;
            case "level_up": return this.level_up;
            case "movement_time": return this.movement_time;
            case "movement_type": return this.movement_type;
        }
    };
    GetSpecificData(_param)//====> Retrive Special data <=================//
    {
        switch (_param) {
            case "high_score": return this.high_score;
            case "success_rate": return this.success_rate;
            case "level": return this.level;
            case "time_to_play": return this.time_to_play;
            case "number_of_questions": return this.number_of_questions;
            case "brain_help": return this.brain_help;
            case "game_instruction": return this.game_instruction;
            case "time_per_question": return this.time_per_question;
            case "number_of_object": return this.number_of_object;
            case "number_of_distractor": return this.number_of_distractor;
        }
    };
    StoreImageData(_this, _db)//===> Storing Image JSON data <====================//
    {
        let location, i, j;
        //========================
        let wholeJSon = JSON.parse(localStorage.getItem("InMySkinImageJson"));//_this.cache.json.get('imageJson');
        let gameJSon = JSON.parse(localStorage.getItem("InMySkinJsonData"));//_this.cache.json.get('jsonData');
        ...
        console.log("bgImageLocatins", bgImageLocatins);
        console.log("normalImageLocations", normalImageLocations);
        GameArchitechture.GameArchitechtureInitiallize();
        //==============cool=================================//
        // for(i = 0 ;i < bgImageLocatins.length;i++)
        // {
        //     location = base_url + bgImageLocatins[i];
        //     LoadAssets.ConvertImageToByte(location,"background_"+i );
        // }
        // for(j = 0 ;j < normalImageLocations.length;j++)
        // {
        //     location =  base_url + normalImageLocations[j];   
        //     LoadAssets.ConvertImageToByte(location,"objects_"+j );      
        // }
