...
class LoadAssets 
{
    ...
    LoaDataFromLocalStorage(_this,_loadAssetsRef,_callback)
    {
        ...
        let wholeJSon = JSON.parse(localStorage.getItem('FindUsIfYouCanJsonData'));
    ...
    {
        let wholeImageJSon =  JSON.parse(localStorage.getItem("FindUsIfYouCanImageJson")) ;
        let wholeJSon = JSON.parse(localStorage.getItem("FindUsIfYouCanJsonData"));
        let bgImageLocatins;
        ...
        {
            image_name = "Back"+_LoadAssets.getImageName(bgImageLocatins[i]);
            // image_name = _LoadAssets.getImageName(bgImageLocatins[i]);
            _this.load.image(image_name,base_url+bgImageLocatins[i]);
            // _this.load.image(image_name,base_url+"background/"+Database.bgType+"/"+colorScheme+"/"+bgImageLocatins[i]);       
        }


        let normalImages = [];
        if(normalImageLocations.length > 27)
        {
            normalImages =  LevelManager.getRandom(normalImageLocations,27);
        }
        else
        {
        ...
        // for (let j= 0;j < normalImageLocations.length; j++) 
        // {
        //     image_name = _LoadAssets.getImageName(normalImageLocations[j]);
        //     if(ObjectType[j] =="emoji")
        //     {
        //         _this.load.image(image_name,base_url+"objects/"+ObjectType[j]+"/"+"warm"+"/"+ normalImageLocations[j]);
        //     }
        //     else
        //     {
        //         _this.load.image(image_name,base_url+"objects/"+ObjectType[j]+"/"+colorScheme+"/"+ normalImageLocations[j]);
        //     }           
        // }
    ...
    LoadCommonItems(_this)
    {
        ...
        _this.load.image('grid', 'assets/images/grid.png');
        _this.load.image('answerBg', 'assets/images/answerBg.png');
        _this.load.image('questionBg', 'assets/images/questionBg.png');
