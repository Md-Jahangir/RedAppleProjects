...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        let headingText = this.add.text(game.config.width / 2, game.config.height / 12, "More Is Less", style1).setOrigin(0.5).setScale(scaleFactorX * 2, scaleFactorY * 2);
        headingText.depth = 2;
        ...
        // graphics1.lineStyle(2, 0xffffff, 1);
        // Showing Lines With Graphics----------------------------------------------------
    ...
    DisplayLevel(_this) {
        ...
        let graphics3 = this.add.graphics({ fillStyle: { color: 0xff0000 } });
        ...
        for (let i = 0; i < this.numberOfAnswers; i++) {
            ...
            let pos = _objectPosition[index__];
            _objectPosition.splice(index__, 1);
            ...
            answers.prevX = pos[0];
            answers.prevY = pos[1];
            // console.log('pos y : ' + pos[1]    +'posx' +       pos[0]);
            // answers.depth = 3;
            // this.objectAngle += 15;
            let circle = new Phaser.Geom.Circle(pos[0], pos[1], 1);
            graphics3.fillCircleShape(circle);
            ...
            console.log(answers.continousEndX + "777777" + answers.continousEndY);
        }
        for (let i = 0; i < this.numberOfDestractor.length; i++) {
            for (let j = 0; j < this.numberOfDestractor[i]; j++) {
                // let index1 = Math.floor(Math.random() * (_objectPosition.length - 0) + 0);               
                let pos1 = _objectPosition[index__];
                _objectPosition.splice(index__, 1);
                ...
                // this.objectAngle += 15;     
                let circle = new Phaser.Geom.Circle(pos1[0], pos1[1], 1);
                graphics3.fillCircleShape(circle);
                graphics3.fillStyle(0x00ff00);
        ...
        if (this.isRotationAvailable) {
            console.log("isRotationAvailable" + this.isRotationAvailable);
            this.RotateImages(this.rotationType);
        }

        if (this.isMovementAvailabe) {
            console.log("isMovementAvailabe" + this.isMovementAvailabe);
    ...
    MovementContineousContineous(_obj, _startX, _startY, _endX, _endY) {
        ...
        console.log("MovementContineousContineous-----------------------", this.movementTime);
        this.tweens.add({
            targets: _obj,
            x: _obj.continousEndX,
            y: _obj.continousEndY,
