class DataBaseManager
{
    ...
    CreateOrOpenDatabase(_this,_LoadAssets,callback,_ref,_callback1)
    {
        ...
        _this.request.onupgradeneeded = (event) => {
            ...
            _this.store = _this.db.createObjectStore('ItsLeftItsRightData', {
                autoIncrement: true
            });
    ...
    InsertContact(_this,name,contact) {
        ...
        // console.log("this",_this.db);
        // console.log("this",_this.request);
        // console.log("this",_this.store);
        const txn = _this.db.transaction("ItsLeftItsRightData", 'readwrite');

        // get the ImageData object store
        this.store = txn.objectStore("ItsLeftItsRightData");
        //
        let query = this.store.put(contact,name);
    ...
    {
        const txn = _this.db.transaction('ItsLeftItsRightData', 'readonly');
        const store = txn.objectStore('ItsLeftItsRightData');
    ...
    GetAllImageData(ref,loadAssetsRef,_this,callback) 
    {
        ...
        _this.request.onsuccess = (event) => {
            _this.db = event.target.result;
            const txn = _this.db.transaction('ItsLeftItsRightData', "readonly");
            const objectStore = txn.objectStore('ItsLeftItsRightData');            
    ...
    {
        _DataBaseManager.request = indexedDB.open('CRM', 1);
        // console.log(" _DataBaseManager.store")
        // create the ImageData object store and indexes
        _DataBaseManager.request.onupgradeneeded = (event) => {
            ...
            // console.log(" _DataBaseManager.store");
            _DataBaseManager.store = _DataBaseManager.db.createObjectStore("ItsLeftItsRightData", {
                autoIncrement: true
            });
    ...
    ClearDataBase(_this)
    {
        ...
        _this.request.onsuccess = (event) => {
            _this.db = event.target.result;
            if( _this.db.objectStoreNames.contains("ItsLeftItsRightData"))
            {
                const txn = _this.db.transaction('ItsLeftItsRightData', "readwrite");
                const objectStore = txn.objectStore('ItsLeftItsRightData');  
            ...
            {
                window.indexedDB.deleteDatabase('CRM',1);
            }        
        }
    };
