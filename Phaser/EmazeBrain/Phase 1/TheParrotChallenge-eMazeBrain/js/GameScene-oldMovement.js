...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        //=============Object Location===========================//
        this.objectPosition = [
            [Math.floor(game.config.width / 12.2), Math.floor(game.config.height / 4.2)],
            [Math.floor(game.config.width / 3.5), Math.floor(game.config.height / 4.2)],
            [Math.floor(game.config.width / 2.04), Math.floor(game.config.height / 4.2)],
            [Math.floor(game.config.width / 1.44), Math.floor(game.config.height / 4.2)],
            [Math.floor(game.config.width / 1.113), Math.floor(game.config.height / 4.2)],
            [Math.floor(game.config.width / 12.2), Math.floor(game.config.height / 2.5)],
            [Math.floor(game.config.width / 3.5), Math.floor(game.config.height / 2.5)],
            [Math.floor(game.config.width / 2.04), Math.floor(game.config.height / 2.5)],
            [Math.floor(game.config.width / 1.44), Math.floor(game.config.height / 2.5)],
            [Math.floor(game.config.width / 1.113), Math.floor(game.config.height / 2.5)],
            [Math.floor(game.config.width / 12.2), Math.floor(game.config.height / 1.75)],
            [Math.floor(game.config.width / 3.5), Math.floor(game.config.height / 1.75)],
            [Math.floor(game.config.width / 2.04), Math.floor(game.config.height / 1.75)],
            [Math.floor(game.config.width / 1.44), Math.floor(game.config.height / 1.75)],
            [Math.floor(game.config.width / 1.113), Math.floor(game.config.height / 1.75)],
            [Math.floor(game.config.width / 12.2), Math.floor(game.config.height / 1.35)],
            [Math.floor(game.config.width / 3.5), Math.floor(game.config.height / 1.35)],
            [Math.floor(game.config.width / 2.04), Math.floor(game.config.height / 1.35)],
            [Math.floor(game.config.width / 1.44), Math.floor(game.config.height / 1.35)],
            [Math.floor(game.config.width / 1.113), Math.floor(game.config.height / 1.35)],
            [Math.floor(game.config.width / 12.2), Math.floor(game.config.height / 1.1)],
            [Math.floor(game.config.width / 3.5), Math.floor(game.config.height / 1.1)],
            [Math.floor(game.config.width / 2.04), Math.floor(game.config.height / 1.1)]
        ];
        //===> Game time <=====//
        this.GameTimer();
        //====> Selecting Background <======//
        this.levelBackground = LevelManager.DecideLevelBackground();
        this.bg = this.add.image(game.config.width / 2, game.config.height / 2, this.levelBackground).setOrigin(0.5, 0.5).setScale(scaleFactorX * 1.5, scaleFactorY * 1.5);
        ...
        if (_bool) {
            switch (_direction) {
                case "RTL":
                    ...
                    _obj.continousStartX = game.config.width; //(game.config.width + (_obj.x/6));
                    _obj.continousStartY = _obj.y;
                    _obj.continousEndX = 0; //-parseInt((game.config.width-_obj.x)/6);//50;
                ...
                case "LTR":
                    ...
                    _obj.continousEndX = game.config.width; //(game.config.width+(_obj.x/6));//(game.config.width+_obj.width);
                    _obj.continousStartX = 0; //-parseInt(game.config.width/2-(_obj.x/6));
                    _obj.continousStartY = _obj.y;
                    _obj.continousEndY = _obj.y;
                    this.movementIntervalX = ((game.config.width) / 10);
                    this.movementIntervalY = 0;
                ...
                case "TTB":
                    ...
                    _obj.continousStartY = this.thumbsUp.y; //-_obj.height;
                ...
                case "BTT":
                    ...
                    _obj.continousEndX = _obj.x;
                    _obj.continousEndY = this.thumbsUp.y; //-_obj.height;
                    this.movementIntervalX = 0;
                    this.movementIntervalY = -((game.config.height) / 10);
                    break;
                case "BRDTL":
                    ...
                    //     objY = element.y;
                    //     // while(
                    //     // (objX > -Math.abs(game.config.width/1.9-element.x))&&
                    //     // (objY > -Math.abs(game.config.height/1.9-element.y))
                    //     // )
                    ...
                    //     }
                    //     // let m = (Math.abs(Math.abs(objY) - Math.abs(element.y))/Math.abs(Math.abs(objX) - Math.abs(element.x)));
                    //     // element.continousStartX = game.config.width;
                    //     // element.continousStartY =((game.config.width - element.x)*m)+ element.y;
                    //     // console.log(  element.continousStartY);
                    //     element.continousEndX = 0;
                    //     element.continousEndY = objY;
                    //     objX = element.x;
                    //     objY = element.y;
                    //     // while((objX <=(game.config.width))&&
                    //     // (objY <= (game.config.height)))
                    //     // {
                    //     while ((objX < (game.config.width + element.prevX / 4))) {
                    //         objX += this.movementIntervalX;
                    //         objY += this.movementIntervalY;
                    //     }
                    //     element.continousStartX = game.config.width;
                    //     element.continousStartY = element.prevY;
                    // });
                    // this.destractor.forEach(element => {
                    //     let objX, objY;
                    //     objX = element.x;
                    //     objY = element.y;
                    //     // while((objX > -(game.config.width/1.9-element.x))&&
                    //     // (objY > -(game.config.height/1.9-element.y)))
                    //     while (
                    //         (objX >= 0) &&
                    //         (objY >= 0)
                    //     ) {
                    ...
                    this.movementTime = ((10000/(game.config.height+200))*( _obj.y));    
                    console.log('movement time : ',this.movementTime);
                    this.movementIntervalX = ((game.config.width)/ 10);
                    this.movementIntervalY = ((game.config.height)/ 10);
                    ...
                    this.answers.forEach(element => {
                        ...
                        let yRight = ((1) * (game.config.width)) + c;
                        console.log('yRight  :  ' + yRight);
                        let xRight = (yRight - c) / (1);
                        ...
                        element.continousStartX = xRight;
                        element.continousStartY = yRight;
                    });
                ...
                this.answers.forEach(element => {
                    ...
                    objX = element.x;
                    objY = element.y;
                    // console.log('val of objX   :  '   +    objX    +     'val of objY   :  ' +   objY);
                    ...
                    let yLeft = ((-1) * 0) + c;
                    console.log('yLeft  :  '  +  yLeft);
                    let xLeft = ((game.config.height) - c) / (-1);
                    console.log('xLeft  :  '  + xLeft);
                    ...
                    element.continousEndX = xRight;
                    element.continousEndY = yRight;
                    ...
                    element.continousStartX = 0;
                    element.continousStartY = yLeft;
                });
                ...
                case "TLDBR":
                    ...
                    this.answers.forEach(element => {
                        ...
                        // while((objX < (game.config.width+(element.x)))&&(objY < (game.config.height + (element.y))))
                        while((objX < (game.config.width)&&(objY < (game.config.height))))
                        {
                            objX  +=  this.movementIntervalX;
                            objY  += this.movementIntervalY;
                        }
                        element.continousEndX = xRight;
                        element.continousEndY = game.config.height;//yRight;
                        
                        // while((objX > -(game.config.width/2 - (element.x)))&&(objY > -(game.config.height/2 -(element.y))))
                        while((objX > -(game.config.width/2))&&(objY > -(game.config.height/2)))
                        {
                            objX  -=  this.movementIntervalX;
                            objY  -= this.movementIntervalY;
                        }                              
                        element.continousStartX = xLeft;
                        element.continousStartY =  this.headerBar.height;// yLeft;
                    });
                ...
                case "TRDBL":
                    ...
                    this.answers.forEach(element => {
                        ...
                        objX = element.x;
                        objY = element.y;
                        console.log('objX : ' + element.x + 'objY : ' + objY);
                        let c = objY - (-1 * objX);
                    ...
                    this.destractor.forEach(element => {
                     ...
                     console.log('objX : ' + element.x + 'objY : ' + objY);
                        ...
                        while((objX > -((game.config.width/2 + element.x)))&&(objY < (game.config.height/2 + (element.y))))
                         ...
                         element.continousEndX = 0;
                         element.continousEndY = yLeft;
                         console.log('element x : ' + element.continousEndX + 'element y : ' + element.continousEndY)
