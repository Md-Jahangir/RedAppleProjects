...
export default class BootScene extends Phaser.Scene {
    ...
    async onDataRecieved(_this) {
        if (Server.platform && Server.platform == "favorites") {
            ...
            if (data.error == 0) {
                ...
                localStorage.setItem("TheParrotChallengeJsonData", JSON.stringify(data.data));
    ...
    ImageData(_data) {
        localStorage.setItem("TheParrotChallengeImageJson", JSON.stringify(_data.data));
    }
    create() {
        ...
        if (checkFirstTime) {
            ...
            if (this.CheckInternetConnection())/*internet is connected*/ {
                ...
                let _data = JSON.parse(localStorage.getItem("TheParrotChallengeJsonData"));
