class DataBaseManager
{
    ...
    {
        _this.request = indexedDB.open('YouAreTheTherapis',1);
        ...
        _this.request.onupgradeneeded = (event) => {
            ...
            _this.store = _this.db.createObjectStore('YouAreTheTherapisImageData', {
                autoIncrement: true
            });
    ...
    InsertContact(_this,name,contact) {
        ...
        const txn = _this.db.transaction('YouAreTheTherapisImageData', 'readwrite');

        // get the ImageData object store
        const store = txn.objectStore('YouAreTheTherapisImageData');
        //
    ...
    {
        const txn = _this.db.transaction('YouAreTheTherapisImageData', 'readonly');
        const store = txn.objectStore('YouAreTheTherapisImageData');
    ...
    GetAllImageData(ref,loadAssetsRef,_this,callback) 
    {
        ...
        _this.request.onsuccess = (event) => {
            _this.db = event.target.result;
            const txn = _this.db.transaction('YouAreTheTherapisImageData', "readonly");
            const objectStore = txn.objectStore('YouAreTheTherapisImageData');            
    ...
    {
        _DataBaseManager.request = indexedDB.open('YouAreTheTherapis',1);
        // create the ImageData object store and indexes
        _DataBaseManager.request.onupgradeneeded = (event) => {
            ...
            _DataBaseManager.store = _DataBaseManager.db.createObjectStore('YouAreTheTherapisImageData', {
                autoIncrement: true
            });
    ...
    ClearDataBase(_this)
    {
        ...
        _this.request.onsuccess = (event) => {
            _this.db = event.target.result;
            if( _this.db.objectStoreNames.contains("YouAreTheTherapisImageData"))
            {
                const txn = _this.db.transaction('YouAreTheTherapisImageData', "readwrite");
                const objectStore = txn.objectStore('YouAreTheTherapisImageData');  
