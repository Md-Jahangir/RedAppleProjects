...
export default new class Utils {
  ...
  getFramesFromJson(jsonpath) {
    let data =  new PIXI.Loader.resources[jsonpath].data
    const frames = []
    for (let t in data.frames) {
      let s = PIXI.Texture.fromFrame (t)
      frames.push (s)
    }
    return frames
  }

  getFramesNumbers(name, totalFrames) {
    let data = new PIXI.Loader.resources[jsonpath].data
    const frames = []
    for (let t in data.frames) {
      let s = PIXI.Texture.fromFrame (t)
...
// Converts from radians to degrees.
  ...
  applyFromPattern(obj1, obj2, isExtra = false) {
    if(!obj1) {
      console.log('MAIN OBJECT ERROR:: ' + obj1 + ' NOT FOUND')
      return
    }
    if(!obj2) {
