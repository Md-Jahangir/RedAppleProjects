...
class LoadAssets {
    ...
    LoaDataFromLocalStorage(_this, _loadAssetsRef, _callback) {
        ...
        let wholeJSon = JSON.parse(localStorage.getItem('ShiftGearJsonData'));
    ...
    LoadDataFromUrl(_LoadAssets, _this) {
        let wholeImageJSon = JSON.parse(localStorage.getItem("ShiftGearImageJson"));
        let wholeJSon = JSON.parse(localStorage.getItem("ShiftGearJsonData"));
        let bgImageLocatins;
        let normalImageLocations = [];
        let image_name;
        console.log("wholeImageJSon", wholeImageJSon);
        ...
        console.log("callback", this.loadCompleteCallback);
        _this.load.on('complete', this.loadCompleteCallback);
    };
    LoadCommonItems(_this) {
        _this.load.image('line_base', 'assets/images/line_base.png');
        _this.load.image('blue2', 'assets/images/blue2.png');
        _this.load.image('unlike', 'assets/images/unlike.png');
        _this.load.image('like', 'assets/images/like.png');
        _this.load.image('ticker', 'assets/images/ticker.png');
        _this.load.spritesheet('gear', 'assets/images/gear.png',
            {
                frameWidth: 97,
                frameHeight: 80
            });
        _this.load.spritesheet('bomb', 'assets/images/bomb.png',
            {
                frameWidth: 202,
                frameHeight: 210
            });
        _this.load.image('borderLine', 'assets/images/borderLine.png');
