...
class Server {
    ...
    async getGameData(_server) {
        ...
        reqHeaders.append("Authorization", _server.token);
        let url;
        if (this.isStandAlone == null || this.isStandAlone == undefined || this.isStandAlone == "") {
    ...
    async sendGameData(_data, _server) {
        ...
        reqHeaders.append("Authorization", _server.token);
        let url = null;
        if (Database.responseUrl == "" || Database.responseUrl == null || Database.responseUrl == undefined) {
            url = this.serverPostAddress;
        }
        else {
            url = Database.responseUrl;
        }
        let option = _data;
