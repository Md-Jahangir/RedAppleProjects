...
export default class BootScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('referenceImage', 'assets/images/referenceImage.png');
    ...
    async onDataRecieved(_this) {
        if (Server.platform && Server.platform == "favorites") {
            ...
            if (data.error == 0) {
                ...
                localStorage.setItem("followthemoscwajsonData", JSON.stringify(data.data));
                console.log('localstorage', localStorage)
                Database.ParseAndStore(data.data);
                this.onImageDataRecieved(data.data);
            }
            else {
                this.CheckDataAvailability(data);
                return false;
            }
        } else {
    ...
    ImageData(_data) {
        localStorage.setItem("followthemoskovaimageJson", JSON.stringify(_data.data));
    }
    create() {
        ...
        if (checkFirstTime) {
            ...
            if (this.CheckInternetConnection())/*internet is connected*/ {
                ...
                let _data = JSON.parse(localStorage.getItem("followthemoscwajsonData"));
    ...
    CheckFirstTime() {
        if (localStorage.getItem('followthemoskovaisFirstTime') == null) {
            localStorage.setItem('followthemoskovaisFirstTime', true);
            return true;
        }
        else {
            localStorage.setItem('followthemoskovaisFirstTime', false);
            return false;
        }
