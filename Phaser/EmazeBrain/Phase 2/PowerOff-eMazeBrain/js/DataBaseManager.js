class DataBaseManager
{
    ...
    CreateOrOpenDatabase(_this,_LoadAssets,callback,_ref,_callback1)
    {
        ...
        _this.request.onupgradeneeded = (event) => {
            ...
            _this.store = _this.db.createObjectStore('PowerOffImageData', {
                autoIncrement: true
            });
    ...
    InsertContact(_this,name,contact) {
        ...
        const txn = _this.db.transaction('PowerOffImageData', 'readwrite');

        // get the ImageData object store
        const store = txn.objectStore('PowerOffImageData');
        //
    ...
    {
        const txn = _this.db.transaction('PowerOffImageData', 'readonly');
        const store = txn.objectStore('PowerOffImageData');
    ...
    GetAllImageData(ref,loadAssetsRef,_this,callback) 
    {
        ...
        _this.request.onsuccess = (event) => {
            _this.db = event.target.result;
            const txn = _this.db.transaction('PowerOffImageData', "readonly");
            const objectStore = txn.objectStore('PowerOffImageData');            
    ...
    StoreImageData(_this,_Database,_DataBaseManager,_callback,_loadComplete)
    {
        ...
        _DataBaseManager.request.onupgradeneeded = (event) => {
            ...
            _DataBaseManager.store = _DataBaseManager.db.createObjectStore('PowerOffImageData', {
                autoIncrement: true
            });
    ...
    ClearDataBase(_this)
    {
        ...
        _this.request.onsuccess = (event) => {
            _this.db = event.target.result;
            if( _this.db.objectStoreNames.contains("PowerOffImageData"))
            {
                const txn = _this.db.transaction('PowerOffImageData', "readwrite");
                const objectStore = txn.objectStore('PowerOffImageData');  
