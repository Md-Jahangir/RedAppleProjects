...
export default class BootScene extends Phaser.Scene {
    ...
    async onDataRecieved(_this) {
        if (Server.platform && Server.platform == "favorites") {
            ...
            if (data.error == 0) {
                ...
                localStorage.setItem("poweroffjsonData", JSON.stringify(data.data));
    ...
    ImageData(_data) {
        localStorage.setItem("poweroffimagejson", JSON.stringify(_data.data));
    }
    create() {
        ...
        if (checkFirstTime) {
            ...
            if (this.CheckInternetConnection())/*internet is connected*/ {
                ...
                let _data = JSON.parse(localStorage.getItem("poweroffjsonData"));
    ...
    CheckFirstTime() {
        if (localStorage.getItem('poweroffisFirstTime') == null) {
            localStorage.setItem('poweroffisFirstTime', true);
            return true;
        }
        else {
            localStorage.setItem('poweroffisFirstTime', false);
            return false;
        }
