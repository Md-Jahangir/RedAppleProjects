...
class GameLogo {
    ...
    create() {
        ...
        this.logo = this.scene.add.image(Math.round(Constant.game.config.width / 2), (40 * Constant.scaleFactorY), "great_blue_heading_with_truck").setOrigin(0.5, 0).setScale(Constant.scaleFactorX, Constant.scaleFactorY);
    ...
    CreateLeftNumberPanel() {
        ...
        for (let i = 0; i < 13; i++) {
            let slotNumber = this.scene.add.sprite(0, Math.round(Constant.game.config.height / 8.2) + (i * Math.round(Constant.game.config.height / 18.5)), "number_" + line[i]).setOrigin(0, 0).setScale(Constant.scaleFactorX, Constant.scaleFactorY);
    ...
    CreateRightNumberPanel() {
        ...
        for (let i = 0; i < 12; i++) {
            let slotNumber = this.scene.add.sprite(Math.round(Constant.game.config.width), Math.round(Constant.game.config.height / 8) + (i * Math.round(Constant.game.config.height / 17.2)), "number_" + line[i]).setOrigin(1, 0).setScale(Constant.scaleFactorX, Constant.scaleFactorY);
            this.slotNumberArray.push(slotNumber);
        }
    }
