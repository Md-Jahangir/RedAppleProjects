...
class Paylines {
	...
	stopSpines() {
		...
		if (this.spineShadowArray.length > 0) {
			for (let index = 0; index < this.spineShadowArray.length; index++) {
				this.spineShadowArray[index].stopShadowWin();
			}
		}
	}
		...
		if (list.length > 0) {
			...
			if (this.currentPaylineIndex < list.length) {
				...
				currentLine.points.forEach((elem, index) => {
					...
					this.spineShadow.playShadowWin(this.spineShadow.usualImg.x, this.spineShadow.usualImg.y);
	...
	CheckWinningStreaks() {
		...
		else if (Model.getLastWin() >= 20 * Model.getBetPerLine() && Model.getLastWin() < 60 * Model.getBetPerLine()) {
			this.scene.jackpotWin.ShowBigWin();
			this.scene.scorePopup.AnimateTweenText(Model.getLastWin());
		}
		else if (Model.getLastWin() >= 60 * Model.getBetPerLine() && Model.getLastWin() < 300 * Model.getBetPerLine()) {
