...
class Reel {
	...
	create(initialSymbols) {
		...
		this.spineBoxContainer = this.scene.add.container(SelectedResolution.width / 2, SelectedResolution.height / 2).setScale(1, 1);
		...
		for (let i = 0; i < initialSymbols.length; i++) {
			...
			// this.spineBoxContainer.add(symbol.winShadowAnim);
	...
	spin() {
		...
		this.scene.time.addEvent({
			delay: this.spinDelay, callback: () => {

				let backInWidth = this.scene.scale.height / 1440;
				for (let i = 0; i < this.symbols.length; i++) {
					if (!this.isFirstPlay) {
						this.symbols[i].initialTween.destroy();
						this.symbols[i].initialTween = null;
						this.symbols[i].initialTween = this.scene.tweens.add({
							targets: this.symbols[i].usualImg,
							props: {
								y: "-=" + ((this.symbols[i].usualImg.displayHeight / 4)),
								duration: 100
							},
							ease: 'Back.In',
							duration: 150,
						...
						this.symbols[i].initialTween.play();
					}
				}
			}, callbackScope: this
		});
