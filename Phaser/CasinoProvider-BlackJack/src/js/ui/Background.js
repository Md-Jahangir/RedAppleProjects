...
class Background {
    ...
    create() {
        if (Constant.isPortrait) {
            let background = this.scene.add.image(Math.round(Constant.game.config.width / 2), Math.round(Constant.game.config.height / 2), "bg_splash_mobile").setOrigin(0.5).setScale(Constant.scaleFactorX, Constant.scaleFactorY);
        }
        else {
