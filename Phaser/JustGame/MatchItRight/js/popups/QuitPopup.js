class QuitPopup {
    ...
    CreateQuitPopup() {
        ...
        var quitTextStyle = { fontFamily: 'Wicked_Mouse_Demo', fontSize: '52px', fill: '#baa6ff', fontStyle: 'bold', align: 'center' };
        this.quitText = this.scene.add.text(Math.round(game.config.width / 2), Math.round(game.config.height / 2), "DO YOU WANT\nTO QUIT ?", quitTextStyle).setOrigin(0.5, 0.5).setScale(scaleFactor, scaleFactor);

        this.yesButton = Utils.SpriteSettingsControl(this.scene, Math.round(game.config.width / 2.5), Math.round(game.config.height / 1.54), "yes_button", 0.5, 0.5, scaleFactor, scaleFactor, "true", this.YesButtonPressed, this.YesButtonReleased);

        this.noButton = Utils.SpriteSettingsControl(this.scene, Math.round(game.config.width / 1.67), Math.round(game.config.height / 1.54), "no_button", 0.5, 0.5, scaleFactor, scaleFactor, "true", this.NoButtonPressed, this.NoButtonReleased);
    ...
    ShowQuitPopup(_refImage) {
        ...
        var alphaTween = this.scene.add.tween({
            ...
            onComplete: function(tween) {
                // console.log("com: ", this.parent.scene.scene.isPaused);
                // this.parent.scene.scene.pause("GameScene");
            }
        });
        // console.log("this sce: ", this.scene.scene.isPaused);
