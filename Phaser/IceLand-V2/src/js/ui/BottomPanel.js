...
class BottomPanel {
    constructor(scene) {
        ...
        this.isAutoSpinButtonText = true;
    ...
    //====================================================================
    CreateBigWinBase() {
        // this.bigWinBase = this.scene.add.image(Math.round(game.config.width / 2), Math.round(game.config.height / 3.6), "big_win_base").setOrigin(0.5, 0.5).setScale(scaleFactorX, scaleFactorY);
        // this.bigWinBase.depth = 2;
        // let bigWinHeadingTextStyle = { fontFamily: 'Roboto_Bold', fontSize: '60px', fill: '#fff', fontStyle: 'bold', align: 'center' };
        // this.bigWinHeadingText = this.scene.add.text(this.bigWinBase.x, this.bigWinBase.y - 45, "WIN", bigWinHeadingTextStyle).setOrigin(0.5).setScale(scaleFactorX, scaleFactorY);
        // this.bigWinHeadingText.depth = 2;
        // let bigWinAmountTextStyle = { fontFamily: 'Roboto_Bold', fontSize: '90px', fill: '#fff', fontStyle: 'bold', align: 'center' };
        // this.bigWinAmountText = this.scene.add.text(this.bigWinHeadingText.x, this.bigWinHeadingText.y + 90, "$ 17", bigWinAmountTextStyle).setOrigin(0.5).setScale(scaleFactorX, scaleFactorY);
        // this.bigWinAmountText.depth = 2;
        // this.HideBigWin();
        this.bigWinBase = this.scene.add.image(Math.round(Constant.game.config.width / 2), Math.round(Constant.game.config.height / 2), "big_win").setOrigin(0.5, 0.5).setScale(Constant.scaleFactorX, Constant.scaleFactorY);
        this.bigWinBase.depth = 2;
        // let bigWinHeadingTextStyle = { fontFamily: 'Roboto_Bold', fontSize: '60px', fill: '#fff', fontStyle: 'bold', align: 'center' };
        // this.bigWinHeadingText = this.scene.add.text(this.bigWinBase.x, this.bigWinBase.y - 45, "WIN", bigWinHeadingTextStyle).setOrigin(0.5).setScale(scaleFactorX, scaleFactorY);
        // this.bigWinHeadingText.depth = 2;
    ...
    ShowBigWin() {
        ...
        //     delay: 100,
        //     scaleX: 1.3 * scaleFactorX,
        //     scaleY: 1.3 * scaleFactorY,
        //     ease: 'Linear',
        //     duration: 300,
        //     repeat: -1,
        //     yoyo: true,
        ...
        this.bigWinBase.setVisible(true);
        let scaleTween = this.scene.add.tween({
            targets: [this.bigWinBase],
            delay: 100,
            scaleX: 1.05 * Constant.scaleFactorX,
            scaleY: 1.05 * Constant.scaleFactorY,
            ease: 'Linear',
            duration: 400,
            repeat: -1,
            yoyo: true,
    ...
    CreateBearAnimation() {
        this.falseReel = this.scene.add.image(Math.round(Constant.game.config.width / 2), Math.round(Constant.game.config.height / 7.8), "bear_anim_reel_bg").setOrigin(0.5, 0).setScale(Constant.scaleFactorX, Constant.scaleFactorY);
        this.bear = this.scene.add.sprite(Math.round(Constant.game.config.width / 2), Math.round(Constant.game.config.height / 2), "bear_run").setOrigin(0.5, 0.5).setScale(Constant.scaleFactorX, Constant.scaleFactorY);
        // this.bear = this.scene.add.sprite(Math.round(game.config.width / 2), Math.round(game.config.height / 2), "symbol_spin_animation").setOrigin(0.5, 0.5).setScale(scaleFactorX, scaleFactorY);
        this.falseReel.depth = 1;
        this.bear.depth = 1;
        this.HideBearAnimation();
    }

    ShowBearAnimation() {
        setTimeout(() => {
            this.falseReel.setVisible(true);
            this.bear.setVisible(true);
            this.bear.play("bear_run");
        }, 100);
    }
    HideBearAnimation() {
        this.falseReel.setVisible(false);
        this.bear.setVisible(false);
        this.bear.anims.stop();
    }
    ...
    OnEnableGUIButton() {
        ...
        if (listOfFreeSpins.length > 0) {
            ...
            if (Model.getLastWin() >= 50) {
                this.ShowBigWin();
                this.ShowBearAnimation();
            }
        }
