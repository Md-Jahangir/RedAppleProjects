...
class Player {
  constructor(_scene, _posX, _posY) {
    ...
    // this.frontCardPosArray = [{ x: -40, y: -100, angle: -20 }, { x: 45, y: -100, angle: 20 }];
    this.frontCardPosArray = [{ x: -90, y: -80, angle: -30 }, { x: -30, y: -100, angle: -10 }, { x: 30, y: -100, angle: 10 }, { x: 90, y: -80, angle: 30 }];
    this.frontCardArray = [];
  ...
  AdjustCharacterSection() {
    ...
    this.userNameText.setStyle({ fontSize: '20px' });
    this.userBalanceText.setStyle({ fontSize: '18px' });
    this.winMessageText.y += 50;
    this.winMessageText.setStyle({ fontSize: '60px' });
    this.winAmountText.y += 50;
    this.chip.y += 50;
    this.winStrength.y += 50;
    this.winStrength.setStyle({ fontSize: '35px' });
    this.frontCardArray.forEach(element => {
      element.y -= 20;
    });
  }
  CreateProfileMask() {
  ...
  CreateNameSection() {
    ...
    const userNameTextStyle = { fontFamily: 'Poppins-Regular', fontSize: '17px', fill: '#FFF3F2', fontStyle: 'normal', align: 'center', wordWrap: { width: this.userNameBase.width - 10 } };
    this.userNameText = this.scene.add.text(this.userBase.x, this.userBase.y, 'Julia Holden', userNameTextStyle).setOrigin(0.5);
    this.playerContainer.add([this.userNameText, this.userNameBase]);
  };
  SetUserName(_name) {
  ...
  CreateBalanceSection() {
    ...
    const userBalanceTextStyle = { fontFamily: 'Poppins-Regular', fontSize: '15px', fill: '#fff', fontStyle: 'normal', align: 'center', wordWrap: { width: this.userBase.width - 10 } };
  ...
  AdjustPlayerInfoSection(_namePosX, _namePosY, _nameBaseXoffset, _nameTextorigin, _offsetXUserBalance, _distFactor) {
    ...
    this.userBalanceText.setPosition(this.userNameText.x + _offsetXUserBalance + 25, this.userNameText.y + 40).setOrigin(_nameTextorigin, 0.5);
    this.userbalanceChip.setPosition((this.userBalanceText.x + _distFactor), this.userBalanceText.y);
    // this.winMessageText.setPosition(this.userBase.x + _namePosX, this.userBase.y + _namePosY - 10).setOrigin(_nameTextorigin, 0.5);
    // this.winAmountText.setPosition(this.winMessageText.x - 75, this.winMessageText.y + 50).setOrigin(0, 0.5);
    // this.chip.setPosition(this.winAmountText.x - 20, this.winAmountText.y - 2).setOrigin(0.5);
    // if (_nameTextorigin === 0) {
    //   this.winAmountText.x += 100;
    //   this.chip.x = this.winAmountText.x - 20;
    // }
  ...
  CreateDelearSection() {
    ...
    const delearTextStyle = { fontFamily: 'Poppins-SemiBold', fontSize: '20px', fill: '#FFFFFF', fontStyle: 'bold', align: 'center' };
  ...
  CreateBackCardSection() {
    this.userBackCard1 = this.scene.add.sprite(this.userBase.x - 40, this.userBase.y - 60, 'front_card_0').setOrigin(0.5).setScale(0.6);
    this.userBackCard1.setAngle(-20);
    this.userBackCard1.setFrame(52);
    this.userBackCard2 = this.scene.add.sprite(this.userBase.x - 20, this.userBase.y - 72, 'front_card_0').setOrigin(0.5).setScale(0.6);
    this.userBackCard2.setAngle(-8);
    this.userBackCard2.setFrame(52);
    this.userBackCard3 = this.scene.add.sprite(this.userBase.x + 5, this.userBase.y - 76, 'front_card_0').setOrigin(0.5).setScale(0.6);
    this.userBackCard3.setAngle(8);
    this.userBackCard3.setFrame(52);
    this.userBackCard4 = this.scene.add.sprite(this.userBase.x + 40, this.userBase.y - 74, 'front_card_0').setOrigin(0.5).setScale(0.6);
  ...
  SetBackCardImageFromServer(_img) {
    ...
    console.log('SetBackCardImageFromServer: ', _img);

    const backCardFrameNum = Utils.GetCurrentBackCardFrame(_img);
    // eslint-disable-next-line no-console
    console.log('backCardFrameNum: ', backCardFrameNum);
  ...
  CreateDecisionBaseSection() {
    ...
    const decisionTextStyle = { fontFamily: 'Poppins-Regular', fontSize: '20px', fill: '#FFF3F2', fontStyle: 'bold', align: 'center'/*, wordWrap: { width: this.userDecisionBase.width - 10 }*/ };
  ...
  CreateSmallBlindSection() {
    ...
    const smallBlindAmountTextStyle = { fontFamily: 'Poppins-SemiBold', fontSize: '20px', fill: '#FFFFFF', fontStyle: 'bold', align: 'center', wordWrap: { width: this.smallBlindBase.width - 10 } };
  ...
  CreateBigBlindSection() {
    ...
    const bigBlindTextStyle = { fontFamily: 'Poppins-SemiBold', fontSize: '20px', fill: '#FFFFFF', fontStyle: 'bold', align: 'center', wordWrap: { width: this.bigBlindBase.width - 10 } };
  ...
  CreateFoldText() {
    ...
    const foldTextStyle = { fontFamily: 'Poppins-SemiBold', fontSize: '20px', fill: '#fff', fontStyle: 'bold', align: 'center', wordWrap: { width: this.foldTextBase.width - 10 } };
  ...
  CreateCurrentBetShowArea() {
    this.currentBetBase = this.scene.add.image(this.userBase.x, this.userBase.y, 'pot_value_chip').setOrigin(0.5).setScale(0.6);
    const currentBetShowTextStyle = { fontFamily: 'Poppins-SemiBold', fontSize: '20px', fill: '#fff', fontStyle: 'bold', align: 'center',/* wordWrap: { width: this.currentBetShowBase.width - 10 } 8*/ };
    this.currentBetShowText = this.scene.add.text(this.userBase.x - 140, this.userBase.y - 60, '', currentBetShowTextStyle).setOrigin(0.5);
    this.playerContainer.add([this.currentBetBase, this.currentBetShowText]);
    this.ShowHideCurrentBetShowArea(false, '');
  };
  ShowHideCurrentBetShowArea(_status, _amt) {
    const amount = _amt >= 1000 ? `${(parseFloat(_amt) / 1000).toFixed(2)}k` : parseFloat(_amt).toFixed(0);
    this.currentBetShowText.setText(amount);
  ...
  AdjustCurrentBetShowArea(_posX, _posY, _offsetX, _originX) {
    ...
    // this.winAmountText.setPosition(this.currentBetBase.x + (3 * _offsetX), this.currentBetBase.y - 4);
    // this.chip.setPosition(this.winAmountText.x - 40, this.winAmountText.y - 2);
  };
  ...
  CreateWinData() {
    const winMessageTextStyle = { fontFamily: 'Poppins-SemiBold', fontSize: '32px', fill: '#F7B731', fontStyle: 'bold', align: 'center', wordWrap: { width: this.userNameBase.width * 3 } };
    this.winMessageText = this.scene.add.text(this.userBase.x, this.userBase.y + 170, 'You Win', winMessageTextStyle).setOrigin(0.5).setVisible(false);
    ...
    this.winStrength = this.scene.add.text(this.winMessageText.x, this.winMessageText.y + 60, '2 PAIR', winStrengthStyle).setOrigin(0.5).setVisible(false);
    const winAmountTextStyle = { fontFamily: 'Poppins-SemiBold', fontSize: '32px', fill: '#F7B731', fontStyle: 'bold', align: 'center' };
    this.winAmountText = this.scene.add.text(this.userBase.x, this.userBase.y, '200k', winAmountTextStyle).setOrigin(0, 0.5).setVisible(false);
    this.chip = this.scene.add.image(this.winAmountText.x - 100, this.winAmountText.y - 2, 'poker_chip_popup').setOrigin(0.5).setVisible(false);
