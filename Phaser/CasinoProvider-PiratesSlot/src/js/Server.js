...
class Server {
    ...
    constructor() {
        ...
        // this.serverAddress = "http://3.16.153.120:5001/api/v1/game-engine/";
        // this.serverAddress = "http://staging.redappletech.com:3021/api/v1/game-engine/";
        // this.serverAddress ="https://staging.bets2win.in/gameapi/api/v1/game-engine";//" http://3.129.109.248:5001/api/v1/game-engine"
        // this.serverAddress = "https://staging.bets2win.in/gameapi/api/v1/game-engine";//" http://3.129.109.248:5001/api/v1/game-engine"
    ...
    async getInitialData() {
        ...
        let url = this.serverAddress + '?' + `game_id=${this.gameId}&action=getInitialData&remote_id=${this.remoteId}&mode=${this.mode}&casino=${this.casino}&currency=${this.currency}&language=${this.language}&token=${this.token}`;
        // let option = {
        //     userId: _userId,
        //     password: _password
        // }
    ...
    async getSpinResults() {
        ...
        if (this.mode == "offline") {
            url = this.serverAddress + '?' + `game_id=${this.gameId}&action=getSpinResult&remote_id=${this.remoteId}&lines=${lines}&coins=${coins}&mode=${this.mode}&token=${this.token}&casino=${this.casino}&currency=${this.currency}&language=${this.language}&balance=${Model.getBalance()}`;
        } else {
            url = this.serverAddress + '?' + `game_id=${this.gameId}&action=getSpinResult&remote_id=${this.remoteId}&lines=${lines}&coins=${coins}&mode=${this.mode}&token=${this.token}&casino=${this.casino}&currency=${this.currency}&language=${this.language}`;
        }
